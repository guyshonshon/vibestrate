...
- **Saga tasks - author a feature as coordinated steps.** A Saga (`vibe saga create`) is a task that holds ordered steps, each with a scoped objective, a done-when check, and file hints. Author the full decomposition of a feature in one card; autonomous step sequencing (the Conductor) is the planned next phase.
