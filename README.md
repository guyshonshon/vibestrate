...
- **Fill your project once.** A Flow declares typed `params:` (name, niche, brand color…); Vibestrate remembers your answers in a durable **project profile** and seeds every later run, so you stop re-typing them. `vibe profile set` / a `VIBESTRATE_PARAM_*` env var is the clean CI seed (a missing required param fails fast, never hangs); the dashboard form prefills, and an optional **Generate** button can have a provider draft a value you review. Secrets are stored as an `env:NAME` reference, never the raw value - and a run fails fast if that env var is unset.
