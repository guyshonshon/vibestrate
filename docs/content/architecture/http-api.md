...
Every method additionally rejects any request a browser marks `Sec-Fetch-Site: cross-site` or `cross-origin`, so a page in your browser cannot drive your local API. Reads are covered as well as writes, deliberately: a browser sends no `Origin` at all for an `<img>` or a no-cors fetch, so the origin rule above never fires on one, and several read routes do real work - provider detection shells out to every coding CLI it knows. `Sec-Fetch-Site: none` stays allowed, since that is you typing the address or opening a bookmark. `same-site` needs its own line: a browser's notion of site ignores the port, so every other server on your localhost counts as same-site with this one. A page there may still link you to the dashboard, but it is refused on `/api/*`, where the side effects are. Non-browser clients such as `curl` omit that header and are unaffected. A destructive endpoint like snapshot prune still requires an explicit body, and never acts on an empty one.
