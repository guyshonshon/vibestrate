...
- **Agents** — the workflow **roles** (planner, architect, executor, fixer, reviewer, verifier) and the **provider** each runs on, plus its permission profile and skills. (An agent is a role; a provider is the CLI it runs on — one provider can power many roles.)
