...
- **mode.** The safety posture the next run will use: `write` (normal) or `read-only` (investigation only, which adds `--read-only`). Press **`m`** to toggle.
...
- **`B`** opens Mission Control (the web dashboard) in your browser.
