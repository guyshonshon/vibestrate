...
Supervised tasks appear as compact container cards on the **Board** page in Mission Control. The task detail view is where you author step objectives, acceptance checks, and file hints - the same structured fields `vibe tasks checklist add` writes. Reordering is available by drag in the detail view (or `vibe tasks checklist move` from the CLI).
