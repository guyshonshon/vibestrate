...
A host you did not list is logged as an exact refusal (`egress DENY connect <host>:443`). A host you did list is refused too if its name does not resolve, resolves to a private or reserved address, or takes longer than fifteen seconds to resolve, and the proxy's log says which. That holds for both paths out: the `CONNECT` tunnel every `https://` call uses, and plain `http://` requests, which are resolved, checked, and then sent to the address that was checked. Only ports 80 and 443 are used, on either path: a `CONNECT` to an arbitrary port is a generic TCP tunnel, not web egress, and an `http://` request to any other port is refused the same way, both of them even for an allowed host. Setting up the network or the proxy is **fail-closed** - if either cannot be created the run is refused, rather than executing with full outbound access while the config claims an allowlist.
