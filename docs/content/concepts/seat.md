...
When a task runs, Vibestrate follows the chain from the step's Seat through your Crew to the actual model and provider: `step.seat` (the chair the step asks to fill) resolves to a Crew Role (the worker in your Crew that lists this seat), which runs on a Profile (the setting that names the model), which names a provider (the tool that runs it).
