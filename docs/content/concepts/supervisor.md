...
A supervisor can carry your stated preferences - "use a hyphen, not an em-dash", "no eyebrow labels" - the kind of rule that is real but not worth a lint rule. On a review turn the reviewer is told to check the change against each and name the fix; a flag rides the normal review and fix loop, the same way a correctness note does. It is advisory, never a separate merge gate, and the reviewer is handed the exact diff so it can actually see a line-level violation. Add one with `vibe preferences add <supervisor> "<rule>"` or from the add field on its card on the Supervisors page - an owner add is live on the next review, no confirm step. You can also just tell the supervisor in a consult ("stop using em-dashes") and it proposes the rule for you; a proposed preference lands *pending* and does nothing until you confirm it (`vibe preferences confirm|reject`, or the Confirm / Reject buttons on the card), so a model can never quietly add a rule the reviewer enforces. Most preferences are advisory - the reviewer flags them and the fix loop corrects them. If you want a *hard* stop, mark one as a block with a pattern (`vibe preferences add <supervisor> <id> "<rule>" --block --pattern "<regex>"`): if the run's diff matches, the run is `blocked` with the reason shown, even on an approved review. A block is deterministic (a regex, not a model verdict) and owner-only. It stays optional: a plain run carries none. The design note is `docs/design/preference-gates.md`.
