...
**The supervisor enforces the project's policies.**
Project rules - "use a hyphen, not an em-dash", "no eyebrow labels" - now live at the project level, not on a supervisor (a project-wide rule must hold under whichever supervisor reviews). The active supervisor carries those rules into the review: it injects the `advise` policies for the model reviewer to check, and the project's deterministic `block` policies cap the merge regardless of which supervisor is active. The supervisor can also *propose* a rule from a consult ("stop using em-dashes"), which lands pending until you confirm it - so a model can never quietly add a rule it enforces. See [Policies](/docs/concepts/policies) for the full surface (tiers, capture, and how the soft rules stay distinct from the hard security gates). Earlier versions scoped these to the supervisor as `preferences`; run `vibe policies migrate` to move an older config.
