...
**Workspace.** The user-level registry of Vibestrate projects you've opened (`~/.vibestrate/workspace.json`). Each `vibe ui` adds its project (and the port it bound); the dashboard's project switcher lists them and hops to another project's dashboard. Projects stay fully independent — each is its own `vibe ui` on its own port — so several can run at once. The **All projects** view (dashboard page, or `vibe workspace overview`) reads each registered project's runs and rolls them up — active/recent runs, outcomes, and spend per project plus combined totals. From there you can also **act** across projects: launch or abort a run in another registered project, or stage runs in a cross-project **dispatch queue** that drains under global + per-project concurrency caps. Cross-project actions only target registered, *initialized* projects (a fail-closed safety gate) and are audited to `~/.vibestrate/workspace-dispatch.ndjson`; the dashboard itself stays rooted in one project. `vibe workspace list/add/remove/overview/run/abort/queue`.
