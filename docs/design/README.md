...
| [`design-system-rollout.md`](./design-system-rollout.md) | **App-wide rollout of the coal/chalk design foundation** (merged to main `f55c8725`). Hybrid plan: verified token crosswalk for primitives/shell, per-surface redesign for screens. 8 phases (0-7); nothing migrated yet beyond Mission Control + control surfaces. |
| [`context-scaling.md`](./context-scaling.md) | Inter-step context / token scaling. **Assessed (2026-06): don't build a code knowledge graph; evolve the existing `FlowContextPacket` when a measured bottleneck appears.** Answers the "Graphy" backlog item. |
