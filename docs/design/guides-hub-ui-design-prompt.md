# Hand-off prompt — claude.ai/design: Amaco "Guides Hub" UI
...
## Design a "Guides Hub" surface for Amaco (Mission Control dashboard)
...
coordinates them through declarative **Guides**. Its dashboard is called
**Mission Control**: a dark, calm, high-density "command center," not a
consumer SaaS app.

### What a "Guide" is

A Guide is a **declarative YAML recipe** (slots + steps) that defines how a team
of agents collaborates on a task — e.g. *plan → implement → review → verify*,
with human approval gates. Guides are small, reviewable data (no embedded code).
Today users fork/customize them in a Flow Builder. They are the unit Amaco wants
to make shareable.

### What the Guides Hub is (what you're designing)

A **registry browser inside Mission Control** — "npm / Docker Hub, but for Amaco
Guides." It lets a developer **discover, inspect, install, and publish** Guides
that other people share. It is **opt-in and read-mostly**: browsing fetches a
public catalog over HTTP; installing writes a guide file locally; nothing
...
Guides are versioned **semver, Docker-style**:
...
1. **Browse / Search (the gallery)**
   - Prominent but compact search field + filters (tags, sort: stars / downloads
     / recent / relevance).
   - A responsive grid (or dense list) of **Guide cards**. Each card:
     guide name, one-line description, author handle, `latest` version chip,
...
2. **Guide detail**
   ...
   - The **install command** shown copyable: `amaco guides install <name>` (and
     the pinned `name:1.2.0` form).
   - A **version selector / history** (list of semver versions, dates, "latest").
   - Readable summary of the guide's **flow**: the ordered steps and which
...
3. **Installed / My guides**
   - List of locally installed hub guides with their **pinned version**,
     "update available → X.Y.Z" affordance, and an **outdated** filter.
   - Actions: update, open in Flow Builder, remove.

4. **Publish flow**
   - A guided form/preview to publish a local project guide: pick the guide,
...
5. **Safety / disclaimer** (recurring component)
   - A persistent, non-alarmist **caution banner** when browsing/installing a
     community guide: *"Using an external guide is your responsibility to
     validate. We review for safety, but be cautious — run untrusted guides
...
- Deliver: the **Browse gallery**, a **Guide detail** page, and the **Safety
