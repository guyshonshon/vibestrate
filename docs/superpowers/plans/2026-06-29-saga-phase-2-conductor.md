...
**Architecture:** A Saga is **one orchestrator run in checklist mode** pinned to the **linear pickup flow** (`checklistSegment {micro-plan..implement}`). Its steps are the checklist items. The Conductor logic is injected at the band's existing **linear** per-item exit seam (`dir === "repeat"`, `orchestrator.ts:4310-4317`), **not** a loop above `orchestrator.run()`. It inherits one-worktree accumulation, per-step commits, resume-by-status, and pause for free. New code lives in `src/feature/`.
