...
> **AS-BUILT (2a shipped):** inline references below to `pickup-review` and the linear seam (`4310`) are SUPERSEDED. Grounding showed plain `pickup` has no per-item review; the design first moved to `pickup-review` (graph review-band), then by owner decision to a dedicated **lighter single-reviewer `saga` flow** (built in M6) - same graph review-band, one reviewer per step. The conductor is injected at the **graph** seam (~`3806`). The authoritative as-built record is [`docs/design/saga-conductor.md`](../../design/saga-conductor.md).
