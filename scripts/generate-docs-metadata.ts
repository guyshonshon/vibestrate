...
function generateWorkflow() {
  ...
  writeJson("workflow.json", {
    schemaVersion: 1,
    description:
      "The default Vibestrate workflow: plan → architect → execute → validate → review → fix → verify. Stages are defined in src/core/workflow/default-workflow.ts; the per-stage prose lives in the docs generator.",
    stages,
  });
}
