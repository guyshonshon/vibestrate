...
import { builtinRoleIds } from "../src/agents/role-schema.js";
import { TERMINAL_STATUSES } from "../src/core/workflow/workflow-types.js";
...
function generateWorkflow() {
  ...
  writeJson("workflow.json", {
    schemaVersion: 1,
    description:
      "Every stage a run can be in, and the status each one enters and leaves on. This is the vocabulary `--resume-stage` accepts, NOT one flow's step list: the `default` flow uses planning, executing, validating and reviewing, while architecting, fixing and verifying belong to `deep`. Stages are defined in src/core/workflow/default-workflow.ts; the per-stage prose lives in the docs generator.",
    stages,
  });
}
