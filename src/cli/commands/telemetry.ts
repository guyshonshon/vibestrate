...
export function buildTelemetryCommand(): Command {
  ...
  cmd
    ...
    .action(async (runId: string) => {
      ...
      const { MetricsStore } = await import("../../core/metrics/metrics-store.js");
      const { RunStateStore } = await import("../../core/state-machine.js");
      const { buildRunTraceOtlp } = await import("../../core/metrics/otel-exporter.js");
  ...
  cmd
    ...
    .action(async (runId: string, opts: { endpoint: string; auth?: string }) => {
      const projectRoot = await root();
      const { exportRunToOtlp } = await import("../../core/metrics/otel-exporter.js");
