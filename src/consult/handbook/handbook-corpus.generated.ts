...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  "schemaVersion": 1,
  "lexicon": "12 127 24 40 40-58 4317 512 58 abort absenc abstract accept acceptanc access across act activ activity adaptiv add-flow add-provider add-skill address adopt advanc advi advic advisor afterward against agent ahead aider aim allow allowlist already alway amp analysi analyz analyze-risk analyze-test anchor annotat another answer anti anti-pattern anyth anywher api apply apply-only approv arbiter arbitrat archetyp architect architectur archiv arg array artifact ask assist assuranc attach attachment attend audit auth authenticat author authz auto auto-pick auto-retri automat automatabl autonomy availabl backend backoff banner bas behind beyond big big-pictur bind bit block blocker board bound boundary branch brief broker brow browser budget builder built built-in bundl button cam cannot cap cap-and-continu capability car card careful catalog caveat cd ceil challeng challenge-respons challenger cheap checklist checkout checkpoint chosen ci claim claud claude-cod clean cleanly clearer cli clo clock cloud cmd co codeba codebas codex coding-agent com command comment commit complet concept conci concurrency concurrent conductor confidenc config configur configurabl configurat confinement confirm conflict connect constraint consult container content context continu continuou contract control control-character convent conversat copy cor correctness cost coverag create-and-run credenti crew criteria cross cross-vendor crush ctrl ctrl-k curat current cursor custom customiz daemon daily dashboard debug-fail deci decid decis decision-summary decomposit dedicat deep deeper definit delay deliberately demand depend dependency deriv derived-flow destinat detach detail detect deterministic diff differ different directory-map disk dismiss distinct doc doctor documentat doesn dollar domain draft driv drop dry dry-run duplicat eaddrinu earlier editor effort egress els email enabl end enforc enforcement engin enhanc enter entirely entry environment ephemer esc escalat estimat event everyth evidenc execut executor exhaust exist exit explain explicit export express extend extern fail-clo fallback fast featur feel fenc fetch fewer field fil filesystem filter fin find finish first first-run fixer flag flaky flat flight flow forbid forc fork form found fram fresh full gap gat gateway gemini gener generat getting-start git git-tree-merg glob glossary go good goos got graph gt guard guid halt hand handoff hard harden head heavier hero high hint history hol hold hom honest host hou http http-api hub human id ids imag implement implementat implementation-review implementer impossibl improv in-app in-progress index info inherit init initiali initializ inject input insid inspect inspect-progress inspector inst installat instruct intak integrat interactiv invariant isolat item itself json judgment jump key kind know knowledg last layout learn least leav ledger leg legibility len lesson library lifecycl limit link listen liv loc localhost localhost-proxy log look loop loopback lt machin mad main manually map markdown matcher materi matter max mcp md ment merg merge-ready merge_ready messag methodology metric micro micro-plan midway min minimalism mod model mov ms narrow nativ navigat ndjson network never newer next noth notif notificat noun objectiv offlin ollama onc opencod operat opt-in orchestrator order os otherwi outcom outsid over-stuf overview overwrit owner owner-only packet panel panel-review param parameter parent part pass past patch path pattern pau paus pause-resum pct per per-part per-pha permanently permiss persona pha pick pick-up pickup pickup-analysi pickup-review pictur pid plain plan plan-only plan-review planner plausibl plausible-but-wrong plu point policy ponytail popular portabl positiv post post-turn postur predict prefix preserv preset press preview proc profil progress project project-param prompt propo protect prototyp prov provider provider-auth provider-nativ proxy prun publish pull push quality quality-arbitrat quest queu quick quickstart quot quota qwen rat rather re re-check re-read re-run re-sequenc re-validat reach read-only readonly ready real really reason recent recogniz recommend recommendat record recoverabl red redact redo refer referenc refin refresh refu register reject remember remot renam reopen reorder repeat replac replay replayabl repo report repository request requir resilienc resolv respect respons restart restat restor resum retent retri retry retyp reu reus revalidat reversibl review review-authz review-correctness review-inject review-item review-risk review-secret review-security-risk review-test reviewer rewind right risk risky roadmap rol room root round rout rul run runtim saf safety saga sandbox say scaffold scan scheduler schema scop scor seat second-review secret secret-scan security security-review seed select separat sequenc servic sess setup sever shap shar sharpen shell shift simplify skill skip slower smaller snapshot snippet someon someth sourc sovereignty spawn spec spec-up spec-up-intak spec-up-review spec-up-roadmap spend split squar src sse ssrf stag standard stat statu stay steer step step-by-step stor strict strong stuck stuf styl subcommand submit subscript subsystem suggest suit summary supers supervi supervis supervised-task supervisor supervisor-control surfac swap switch synthesiz tab taken taker target task task-lifecycl teach team telemetry tell termin test thorough thr threshold throw tighten timelin timeout tip titl token touch tour transient tre troubleshoot try ts tun turn twic two ui unassign unattend unavailabl uncommit undo uneven unprotect unsaf until upload url usag usd validat vendor verb verbatim verificat verifier verify vib vibestrat vibestrate-md vibestrate_param visibl vocabulary wait waiting_for_approv walk walkthrough wall warn watch weak week welcom whether whol why-a-human window without workflow workspac worktr worktre wrot yaml yet yml yourself zero zero-egress",
  "entries": [
    {
      "id": "cli/abort",
      "kind": "cli",
    ...
    },
    {
      ...
      "terms": "0 1 127 20 4317 4400 8 a abort accept acm add advanc advis agent allow allow-token-to-custom-host an and any api apply approv architect architectur are area arg as ask assess assign assuranc astro audit auth automat automatabl bas base-url befor boolean browser built built-in cd chang cheap check clear cli cod codebas codebase-map com command config configur consult content control convent cor could crew current custom dashboard deep deep-review deeper default definit descript detail detect did discoverabl do doctor don draft enabl enter env every execut export fals fetch fil filter find first fix flow follow for framework friendlier from gat generat get git github going group guidanc h handl healthz heavy high host how hub i id import in init input insid inspect inst interactiv is it its json key know l learn leav ledger level list log login look loop machin map max md miss mor my my-flow nam new no no-open not noun null number of oldest on onc one only open or out overview overwrit param parameter pass past path paus payment per plan planner pnpm port preset preview print profil project project-param provider publish raw re re-run read read-only readabl redo referenc regenerat reject remov renam render replay request request-chang resolv resum resume-from resume-stag reus review review-heavy reviewer rewind right risk rout run s sam say scaffold schema semver set settabl setup shap shell show simpl sk skill slug so spend stag start statu step stor strong subcommand suggest supervis supervisor t tabl task termin test that the then thorough tip to token top top-level tru typecheck ui unassign unset up url usd use user validat verb verify vers vib vibestrat vibestrate_api_token vibestrate_hub_token view wait welcom what wher wir with word work workspac worktre writ wrong yaml yes yml you your your-github-login zod",
    ...
    },
    {
      ...
      "terms": "0 1 1-9 2 3 4 5 6 7 8 9 a abort activ activity advanc alway always-on an and approv argument autocomplet automat b back bar body branch browser budget c cap cli clos command complet config consult context context-sensitiv control crew ctrl current d daily dashboard deeper default delet did dismiss doc doctor down e edit effort end enter esc f flag flow get git going h header help hi high history hom i idl ids in in-termin it its j json k key know layout liv low m main medium miss mod n nam navigat new no notif o of on only open opt or p pag palett panel paus pick prefix previou profil project prompt q queu quit r re re-run read read-only replay resum retent roadmap run s sam scroll select sensitiv sess set shell shift show simpl skill snapshot spac spend statu subcommand suggest switch tab task termin the thing tip to toggl topic trunk up usd validat valu vib vibestrat view websit what with word worktre writ you your",
    ...
    },
    {
      ...
      "titleTerms": "annotat",
      "terms": "0 1 127 40 40-58 4317 58 a actually add agent an annotat as auth authoritativ bear befor bug codebas concept copy deeper did do don fil for generat going guidanc her human in inspector is it json know liv load load-bear not one order pattern pin privat project read refactor resolv rol see sess short simpl src start t task teach the them thes they thi tip to touch treat ts ui user vib vibestrat visibl what when wher which word work worktre would you your",
    ...
    },
    {
      ...
      "titleTerms": "consult",
      "terms": "a about an and answer api apply ask assist at behind beyond block can caveat chang concept confidenc config confirm consult context cost deeper did diff do documentat doe estimat fil fit flow from get git going good ground guid heavier her how i id in is it its key know last leav left list materi md me money new noth null one orb pag pass policy post profil project propos quest r really recommend reject relat run s screen shell should show simpl sourc src task termin that the their ther thi tip to ts two until updat use vib vibestrat what wher which why word writ x yml you your",
    ...
    },
    {
      ...
      "titleTerms": "crew",
      "terms": "0 1 127 3 4317 a add ai all all-loc an and another builder by careful cast chang cheap check claim concept configur cor crew deeper default did draft each edit entirely every executor fast from going hand hav implementer in init insid install it know list loc mad mark model never new no of offlin on one order pag preset print profil project propos ready ready-mad remov rol roster run sav seat seat-rol second set shell show simpl tak task team termin the thorough tip to two ui use vib vibestrat which why word work worker would written yml you",
    ...
    },
    {
      ...
      "titleTerms": "flow",
      "terms": "0 1 127 2 4317 a actually adaptiv add alway and arbitrat as ask assist auth automat brief build builder chang check clear cli codex codex-review com concept contain continu cor customiz dashboard deeper default definit delet deriv did draft dry each edit enter error export express f field flow follow for fork from gat going h has hub import in install it its keep kind know list mak model nam need new no no-select of off on one only open or order overrid own past paus pick plan plan-only profil project provider publish pull quality quality-arbitrat recip redo referenc repeat restor reveiw review reviewer run sam sav seat select set shar shell should show simpl siz skip spec step step-profil termin the thi tighten tip to ui undo unknown up use vib vibestrat want what when wher which word worker would writ yaml yml you",
    ...
    },
    {
      ...
      "titleTerms": "policy",
      "terms": "0 1 12 127 4 4317 a add adopt advic advis against an and automat block both broken character concept config confirm consol consult dash deeper deterministic did discard do doctor draft em em-dash enforc engin english every eyebrow fix from gat going guard hard hyphen id in integrat it know label list log matcher merg migrat new no no-em-dash no-eyebrow not on one only owner owner-only persona policy policy_advis policy_block preferenc project prompt propos recent reject remov rul run s safety sect see shell sid simpl snippet soft stop suggest supervisor surfac test the thi tier tip to ui use vib vibestrat what word would writ yml you your",
    ...
    },
    {
      ...
      "terms": "abstract actually agent already an and away be beyond build can chang concept config deeper dependency did diff do doe execut exist fals featur fewer gener going her in inst is it know library lin mak nativ need not one only over over-build ponytail postur provenanc quest relat run set simpl smaller smallest standard stop task that the then thi tip to trad trust vib what why will word work writ writer you",
    ...
    },
    {
      ...
      "titleTerms": "provider",
      "terms": "0 1 11434 127 4 5 a accept actually add advanc agent ai ai-compatibl along also an and anthropic anthropic-api anthropic_api_key api apply are as bas built built-in c capability catalog claud claude-cod claude-haiku claude-pro claude-sonnet-4-5 clear cli cloud cod code_writ coding-agent com command compatibl concept config configur cor crew custom deeper default destinat detect did doctor driv dry dry-run eco edit effort egress endpoint entry env env-ref environment exist explicitly extern family finetun flag for forc format four from further gemini going haiku help http http-api if in install is it json key kind knob know level list liter loc localhost localhost-proxy login machin mak md miss mod model my my-finetun mycli nam need never no not noth null ollama ollama-loc on only open openai option or order output overlay own p permiss permission-mod popular preset pro prob profil project prompt provider providers-catalog proxy qwen3 r raw real reason ref refresh remov replac run saf safe-mod sav server set setup shell show simpl smok sonnet sourc sovereignty submit suggest termin test the thi tip to token tru turbo typ up url usag use user valu vib vibestrat what which wir with word would writ yaml yml you your zero zero-egress",
    ...
    },
    {
      ...
      "titleTerms": "run",
      "terms": "0 1 127 4317 a abort approv assuranc at attempt automat block by chang changes_request concept control copy cor creat crew deeper describ detail did diff driven end execut fail fil fix flow for going how human id in inspect is isolat its know liv merg merge_ready metric miss need needs_human of on one order own pass paus re re-run read ready record replay repo request review run sam shell simpl statu task termin the thre through timelin tip ui validat verdict vib view wait waiting_for_approv watch what with word workspac you your",
    ...
    },
    {
      ...
      "terms": "0 1 120 127 40 4317 a act against allow allowlist already anchor and any api applicabl apply apply-only approv are artifact as ask assuranc at attend audit auto auto-retri automat autonomy backend backoff banner befor block blocker bound boundary broken broker budget but cap ceil chang changes_request check checkpoint claud clear clock codex command complet concept confidenc config confinement container control count creat crew cross daily day decid dedicat deeper default defens deny depth descript deterministic did diff docker doctor doe dollar dotenv down downgrad downgrade-model duplicat dur effect effort egress engin env environment every exec execut exhaust exit fail fallback fetch fil fix flag flow for gat get git glob going guard hard harden harden-read-only has hiccup hold hold-merge-for-review i id in inert init inspect install is isolat it json kind know lik limit list load match max max-time-day max-turns-run mcp md merg merge_ready messag metric min miss mod model nam nativ ndjson need network new no no-network-install no-secret-writ non non-zero not not_applicabl not_run noth npm off on one only or os pars parti partially partially_verifi pass past patch path paus per permiss permission-mod pip plan policy post post-turn postur profil project provider provider-nativ re re-run read read-only ready real reduc reduce-effort refu refus regex reject render report request requir require_approv resilienc retri review root rul run s safety sam sandbox scor seat secret set shap shell show sign simpl skill skip skipped_inert_diff spawn spend start statu stop strict strict-apply-only subscript supervisor tab termin the thi throw tim tip to today tool tool_us tre tru turn ui unattend unbound unbounded_unattended_run unsaf usag usd use validat verdict verifi vib vibestrat wait wall what which with without word worktre writ yml you your zero",
    ...
    },
    {
      ...
      "terms": "0 1 127 4317 a accept acceptanc adaptiv all an answer anyth approv architectur ask auto auto-pick befor beyond board brief build by cannot card chain chosen command complet concept coverag creat deeper did dismiss draft dry dry-run earn ecommerc edit editor els fil flow forc from gap get going hand her hol honest id ids in intak into it item its itself keep know limit matter mini new next no no-select not off on one otherwis pag pick proc propos prototyp quest ready register remember review risk roadmap round run s sav scop seed select shell simpl simplify someon spec spec-up spec-up-intak start stor submit suggest synthesi synthesiz termin the tip to tre turn twic ui up use vagu valu vib week what when why will with word work would written you your",
    ...
    },
    {
      ...
      "titleTerms": "run stat",
      "terms": "a abort agent agent-rais all allow allowed_transit and approv architect are ask at automat between block c chang concept cor creat dashboard deeper detail did each enforc error execut fail filter fix for from gat going guidanc honest id in is it json keep kind know list look mark mean merg merge_ready mov of on one only open order paus plan r rais ready recoverabl reject replay request request-chang requir resum review rul run sam shell simpl stag stat statu termin that the them tip to transit two validat verify vib wait waiting_for_approv what wher why word worth you",
    ...
    },
    {
      ...
      "terms": "0 1 127 20 4317 a acceptanc add all and at audit author between beyond board by card check checklist cleanly clear concept conductor config context curat deeper did dismiss do doe enabl enhanc escalat fals fil fresh from goe going ground halt hint how id in instead invariant it know ledger log max mod model new not objectiv of on one only or order packet pass paus plain plan plan-only profil project re re-ground re-sequenc refin relat remov reorder resum roadmap run sequenc shell simpl spend src statu step supervis supervised-task supervisor task termin the tim tip ts ui usd vib vs what with word writ writer yet yml you",
    ...
    },
    {
      ...
      "titleTerms": "task",
      "terms": "0 1 127 4317 9 a acceptanc add adjectiv adjective-noun all and append apply assuranc attach audit author auto auto-pick automat becom block board bold bold-lovelac break brief but by c cap cap-and-continu card chang check checklist cli comment concept configurabl constraint continu continuou cor correctness crew criteria d deeper deriv detach detail did dismiss don draft e enhanc enough enter exist flow forc from going good hand handler health how human id improv in in_progress includ inherit instead into is it item its json key know languag ledger len lib list log logger look lovelac manually matter merg merge_ready mov n nam need never new not noun one only open order out own parent pend per per-item pick pickup pickup-review plain plain-languag plan plausibl plausible-but-wrong progress propos q r read read-only ready reopen return review risk roadmap rout run sav say security security-risk select sentenc server set shell should simpl src start statu step step-by-step structur supervis task termin test that the then tip titl to ts ui use user valu vib vibestrat what when whol will word work writ wrong yet you",
    ...
    },
    {
      ...
      "terms": "a against and append apply arbitrat ask at author be best bil build careful codebas codebase-map command commit concept constraint consult convent credenti crew deeper development did do effort els entry execut explain express flow for generat get go goe going good guid guidanc head heavier how http id implementer in init install is it json know known layout learn ledger lesson machin machine-own manu map md mod model money never not noth onc one only open or orchestrat order other own path planner pnpm point policy prefer preferenc print project propos provider quality quality-arbitrat r rank re re-explain read refresh relat review risk rol root rout rul run s sandbox scaffold servic shell should show simpl so someth src starter surfac task teach test text that the thi through tip to touch typecheck ui use vib vibestrat vibestrate-md what when who word work writ x you your",
    ...
    },
    {
      ...
      "titleTerms": "worktre",
      "terms": "a abort absolut after along are at auto automat block bold bold-lovelac branch bring buy cd chang checkout concept copy cor d deeper default did diff dir each env environment every evidenc except fail failur fenc fil git going honest id in keep know link liv lovelac main merg merge_ready modul never nod node_modul noth of off onc one only order path pem prefix project quiet quiet-tur ready real remov run see separat simpl so sourc termin the their thi tip to tool touch tur undo venv vib vibestrat vibestrate-worktre view what wher word work workspac worktre writ yml you your your-project",
    ...
    },
    {
      ...
      "terms": "0 1 127 4317 a act add add-flow agent agent-turn an and approv approval-gat as befor behind both brief brows build builder but can challenger chang check clean clean-room clearer com command commit common continu creat customiz decid deeper descript did diff draft dry edit exampl export extend f fil first flow flow-skip for fork form from gat going h http hub human human-check id import in input keep kind know label list mistak model narrativ new not on one only open option or out output over over-stuf overwrit own past paus plan planner producer profil project prototyp prototyper pull read read-only reason recip relat request respons response-turn review review-turn reviewer rewrit rol room run s seat seat-rol shar shell simpl skill skip spec spik spike-and-decid step step-profil stop stuf summary summary-turn taken task task-brief the then tip to tru try turn twic ui url validat vers vib vibestrat view with word writ yaml yml you your",
    ...
    },
    {
      ...
      "terms": "0 1 127 2 3 4 4317 a about access add add-skill agent an and anti anti-pattern api api-convent arg assign attach auth auth-convent be beat body bound bullet check claud command convent creat crew deeper default descript did directory discover editor exampl explicitly extend fa fetch fil flat flow for form going good grant id in inspect is it json keep know list mak mark markdown mcp md ment mkdir mor nam not of one only option or p pattern payment permanently permiss pg pg-mcp planner plu postgr prefer profil project prompt query read read-only reason relat requir right rol rul run sav seat sentenc server sess shap shell short show simpl skill specific src stat stay step surfac that the thi tip titl to ts two ui unassign under use vib vibestrat was way we what when which with word writ x yml you",
    ...
    },
    {
      ...
      "terms": "5 a abort about add adjectiv adjective-noun anyth approv artifact assuranc auto auto-pick behind big block bohr branch can cd chang concis configurat copy crew decis deeper did diff don end event fail fin first first-run flow for from get getting-start git going handler hom how id in input into it know leav log main md merg merge_ready ndjson never new next noun one or order output p pass pick plan policy r ready replay review right roadmap run sav scop sentenc set short simpl small sourc start statu step stop structur t tab tak task termin that the tip to too tre typ ui unattend up use validat verificat verify vib vibestrat vibestrate-worktre watch well well-scop wher word work workspac worktre you your zen zen-bohr",
    ...
    },
    {
      ...
      "terms": "0 1 127 2 24 4317 5 a add agent an and as at cd check ci cli cod codebas coding-agent com config connect creat curl dashboard deeper did doctor edit empty f failur first fix flow folder for from fs g get getting-start git git-init githubusercontent gitignor going guyshonshon http in init initialis insid install installat is it js json know learn least let machin machine-readabl main md model mor newer next nod npm one only open or order pag pass pnpm point policy project provider publish r raw read read-only readabl render repair repo report requirement rol rul run s saf sam scaffold set setup sh simpl skill skip sl start statu tak termin test the thi thing tip to too ui up url vers vib vibestrat view warn what word writ yml you your your-project",
    ...
    },
    {
      ...
      "terms": "a across advic advis ahead alway analyz and any apply ask behind best bold bold-lovelac branch call cd chang checkout complet confirmat conflict copy creat deeper deterministic did diff dry dry-run each ff ff-only finish for from get getting-start gh git going gt guid how id in integrat into is it json keep know leav loc locally lovelac lt machin main merg merge-ready merge-to-main merge_ready minut model mov nam next not of on one only open opt option or order part plan planner pr predict preview project pull read read-only ready real report request run s sam shar simpl sourc start tak target termin the then thi thre tip to tre typ undo vib vibestrat vibestrate-worktre what word worktre you your",
    ...
    },
    {
      ...
      "terms": "0 1 11434 127 2 4 4096 5 6 a add agent ai aider already among an and anthropic anthropic_api_key api are arg at bas claud claude-sonnet-4-6 cli cloud cod codex codex-default com command confident configur connect crew cross cross-model custom deeper default detect did different doctor edit env every executor fix for found from get getting-start giv going hav how http http-api implementer in init inst install is it its key kind know liter loc localhost localhost-proxy login machin mak max model mor nam need never next not ollama on one only open opencod opt option or order p pag path point popular print profil project prompt provider proxy qwen3 ready referenc reviewer run runtim s saf sam server set setup simpl singl single-profil smok someth sonnet start stay stdin tab termin test the they thi thre tip token tool tri typ up url v0 v2 vers vib vibestrat what wher with wizard word yml you your",
    ...
    },
    {
      ...
      "terms": "0 1 127 2 227 24 3 4 4317 4318 5 6 a accept acm acme-api add address advic advis ahead ai aider aider-install already analyz and anthropic anthropic-ai anthropic_api_key api apply approv arbitrat are arg as assuranc at auto auto-pick availabl back bashrc behind bin block bold bold-lovelac branch brief by can cd chang check checkout claud claude-cod cli cloud cod codebas codex command commit complet concis config configur configurat confirm connect copy crew custom dashboard deeper default detect did diff doctor doe each eaddrinus ebadengin echo edit els endpoint enter env everyth execut exit export express extra fail fil fin find finish first fix flaky flow for fork found from g gemini gemini-cli get getting-start git git-init gitignor going googl handler happen hav head healthz help her http http-api id improv in init initialis initializ insid install integrat into is isn isolat it js just keep key know learn let lik list listen loc localhost localhost-proxy lock log login look lovelac m main manager md mean merg merge-to-main mod model mor mov nam need new newer nod not npm nvm of ok on one only open openai openai_api_key operat option or outcom outsid own pag panel panel-review pass path permiss permission-mod permit pick pip plan plan-only pnpm point policy popular port prefix project prov provider proxy python quality quality-arbitrat quickstart re re-run re-test read read-only ready refere repo repository restart result review rol rul run s saf sandbox sav scaffold scop security security-review see sentenc server set setup sign simpl skill sourc start statu stay strict structur t tak task test that the then thi tip to tru trunk typ typecheck ui ui-port unattend uncommit until up use v2 validat verificat vers vib vibestrat vibestrate-worktre vibestrate_provider_ok view wait warn what when which why with won word work workspac workspace-writ worktre worth writ yaml yes yml you your your-project zshrc",
    ...
    },
    {
      ...
      "terms": "2 8 a about add agent an and are assign attach auth auth-convent balanc carry chang claud claude-balanc codebas convent cooky creat crew deeper default did domain don enrollment error error-handl every fa fetch for from get getting-start go going hand handl her how http id in inlin into is it json jwt keep know lax list login lucia markdown mcp md middlewar mint modul mor nam never new next not on one only order path permiss planner profil project prompt read read_only really reason requir restat review rol rout rul run s sam seat second security security-review server sess show sign simpl sit skill skip spac src start subsystem t termin that the thi thos tip to touch ts unassign under url use vib vibestrat vocabulary what when with word work worth writ yml you your",
    ...
    },
    {
      ...
      "terms": "0 1 127 4317 a add agent ai already and api audit boundary build call can chosen cli cod consult context context-url copy crew dashboard deeper default did doc end environment every express fin flow full get getting-start git go going hav her http http-api id in index init is it know leav link log model modul next nod node_modul not of off on one only per plan profil project quest quick quickstart re re-read read read-only recip record repo review reviewer rul run scaffold set shar simpl skill start statu stay steer task termin that the thi tip to together ui unattend url venv verify vib vibestrat walkthrough want what when wher word work worktre writ you your",
    ...
    },
    {
      ...
      "terms": "1 2 25 3 4 5 50 a abandon abort add advanc advis advisor alongsid analyz and arbitrat artifact audit auto auto-pick automat away backoff behind branch by can cd chang checkout cli complet concis config configurat creat create-and-run crew dashboard decis deeper default did diff different don every ff ff-only fil finish finish-now first flow for fram from gh git go going good heavier how human id in input inspect integrat is it json just key know lib list liv locally log logger main md merg metric model need new now oldest on one only or output path permiss pick pr preserv profil project protect push quality quality-arbitrat re read read-only relat remov replay resolv resolve-first result retry review rout run server set shar simpl sourc src stag stage-on-integration-branch start statu suggest supervisor task termin than the thi thing thre timelin tip to touch tru ts tun ui unattend uploader vib vibestrat vibestrate-worktre walk watch weak when whether with word workflow worktre yml you yourself",
    ...
    },
    {
      ...
      "terms": "20 a advanc after and api architect architectur artifact authenticat automat banner block bug cd chang clean cli command creat crew debug debug-fail decis deeper did diff do doesn drop dry dry-run earlier end event every execut exist fail figur fil find fix flow from get git going how id if in instead is json just keep know list look main md miss ndjson new newest next not noth of old or orphan out outcom output over per per-phas permiss phas plan post preview project provider prun re re-run ref referenc relat replay requir restart restor restore-preview resum resume-from resume-stag retent reus review rewind rul run s sam scop seed sharpen simpl skill snapshot sourc stag start stat statu stderr stdout step step-id t task teach test the thi tighten tip to unsaf validat verificat verify vib vibestrat what when wher why with without word workflow worktre y yml you",
    ...
    },
    {
      ...
      "terms": "a add advanc advis already and apply as ask automat befor branch clean cli click commit confirm conflict dat deeper did do doe down dry dry-run env every explor ff fil finish for git git-tree-merg going good graph guid head her history id in inspect inspector integrat into is it json know let log main merg merge-ready merge-to-main mov nam never no no-ff not of one only open or path per per-run planner predict preview propos push read read-only ready redact remov report resolv run see shap simpl sourc supervisor target the thi tip to tre undo up vib what whol with word workflow would you your",
    ...
    },
    {
      ...
      "terms": "5 a act advanc and append append-only artifact as automat bold bold-lovelac broker cd chang cli cod command cost creat current dashboard decis deeper deni detail did disk durat event every execut exit fil flow follow for going happen id in inspect inspect-progress interactiv is it jq json know liv log lovelac main md merg merge_ready messag metric n ndjson new on one only output p participant past per plan profil progress prompt provider r raw read ready relat replay resolv respons result review rol run runtim runtime-metric s sav seat select shell simpl snapshot sourc stat statu stderr stdout step step-id stream tab termin the thi timelin tip to token transit txt typ ui validat validation-result verdict verificat verify vib vibestrat watch wher word workflow workspac you",
    ...
    },
    {
      ...
      "terms": "a abort advanc and approv are at automat back block branch bring cd chang cli control d deeper detail did different end for from gat git going good guidanc how human human_approv id in is it know later let list max miss next on or p pau paus pause-resum pick point policy policy-gat project r refus reject remov request request-chang requir resum resume-from round run saf safely simpl someth stag statu stop the thre tip to up vib vibestrat vibestrate-worktre wait waiting_for_approv way what wher word workflow worktre you your your-project",
