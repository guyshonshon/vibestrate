...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "terms": "a add ai and any api architect at auth behind big big-pictur builder but chair challenger check cli cod crew deeper default did don each executor fil fill flow for get getting-start glanc going handler has hav hold how http http-api implementer in init it job key know label localhost localhost-proxy log machin mean meet model mor need no of on one open or overrid own pick pictur profil provider proxy reviewer rol routin run s sav seat seat-rol senior senior-reviewer server set simpl start strong structur t task team than that the thi through tighten tip to tool up vib vibestrat want what will with word worker you your",
    ...
    },
    {
      ...
      "terms": "a abort about add adjectiv adjective-noun anyth approv artifact at behind big block bohr branch can cd chang creat decis deeper did diff don end event fail fin first first-run flow for from get getting-start gh git going handler hom how in it know leav log look main md merg merge_ready ndjson never next noun one or output pass pick pr ready review right run sav scop sentenc set short simpl small sourc start statu step stop structur t tak task termin that the tip to too typ ui use verificat verify vib vibestrat vibestrate-worktre well well-scop what wher word work worktre you zen zen-bohr",
    ...
    },
    {
      ...
      "terms": "2 a about add agent an and are assign auth auth-convent balanc carry chang claud claude-balanc codebas convent cooky creat crew deeper default did domain don enrollment error error-handl every fa for from get getting-start go going hand handl her how http id in inlin into is it json jwt keep know lax list login lucia markdown mcp md middlewar mint modul nam never new not on one only path permiss planner profil project prompt read read_only really reason requir restat review rol rout rul run s sam seat second security security-review server sess show sign simpl sit skill skip src start subsystem t that the thi thos tip to touch ts unassign under use vib vibestrat vocabulary what when with word work worth writ yml you your",
