...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "titleTerms": "flow",
      "terms": "a adaptiv add advanc and arbitrat as ask assist at auth automat check choos clear cli codex codex-review com concept contain continu cor crew customiz deeper default definit deriv draft each edit error every express field fil first flow follow for from going has hub import in install instruct it its keep kind list mak model nam need new no no-select of off on one open or order other overrid own pick point profil project provider pull quality quality-arbitrat recip referenc reveiw review reviewer rol run s sav select set show siz skip spec step step-profil the then thi tighten unknown up use vib vibestrat what when worker writ yml you your",
    ...
    },
    {
      ...
      "titleTerms": "worktre",
      "terms": "a abort absolut advanc after along and are auto automat block bold bold-lovelac branch bring cd checkout cli concept copy cor d deeper default diff dir doe env environment every fail fil git going id in its keep link liv lovelac main merg merge_ready modul never nod node_modul of off only order path pem prefix project quiet quiet-tur ready real remov run saf separat so the thi tool touch tur venv vib vibestrat vibestrate-worktre view wher why work workspac worktre yml you your your-project",
