...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  "schemaVersion": 1,
  "lexicon": "110 12 120 23 24 322 40 40-58 512 58 abort absenc abstract access act activity adaptiv add-flow add-provider add-skill advanc advi advisor advisory agent aider aim allow allowlist already alway amp analysi analyz analyze-risk analyze-test anchor annotat another answer anti anti-pattern anyth anywher api apply apply-only approv arbiter arbitrat architect architectur archiv arg array artifact ask assign assist assum assuranc attach attachment attend audit auth authenticat author authz auto auto-retri automat autonomy availabl backend backoff banner bas becom behind best-effort big big-pictur bit block blocker board bound boundary branch brand brief broker budget builder built built-in bundl button cannot canva cap cap-and-continu capability car careful catalog cau caveat cd ceil challeng challenge-respons challenger cheap checklist checkout checkpoint claim claud claude-cod clean cli clo clock cloud cmd co code_writ codeba codebas codex coding-agent com command commit complet concept concurrency concurrent conductor confidenc config configur configurabl configurat confinement confirm conflict constraint consult container context continu continuou contract control control-character convent conversat copy cor correctness cost create-and-run creator credenti crew cross cross-vendor crush csrf ctrl ctrl-k curat cursor custom customiz daemon daily dashboard debug-fail deci decid decis decision-summary decomposit dedicat deep deeper default-allow delay deliberately deni deny depend dependency deriv derived-flow destinat detach deterministic diff differ different directory-map distinct doc doctor documentat doesn dollar domain downgrad downgrade-model draft drop dry dry-run duplicat earlier early editor effect effort egress els email enabl end enforc enforcement engin enhanc enter entirely env environment ephemer esc escalat estimat event everyth evidenc execut executor exhaust exist exit expect explain explicit export express extend extern fail-clo fallback fast featur feel fell fenc fetch fewer field fifty fil filesystem fill filter fin finish first first-run fixer flag flaky flight flow forbid form fresh full gap gap-fill gat gateway gemini generat getting-start git git-tree-merg glanc glob glossary going good goos got guard guid halt hand handoff hard harden head headless heavier held hero high hol hold hom honest horizont host hou http http-api human id ids imag implement implementat implementation-review implementer improv in-progress index info informat inherit init initializ inject input insid inspect inspect-progress inspector inst installat instead instruct intak integrat interactiv invariant isn isolat item itself job json judgment key kind know knowledg label last leak learn least leav ledger legibility len lesson library lifecycl limit link liter liv loader loc localhost localhost-proxy log look loop loopback loudly machin mad main map markdown match matcher materi matter max mcp md ment merg merge_ready messag methodology metric micro micro-plan min minimalism mod model mov ms narrow nativ navigat navigator ndjson network never newer next non non-cli non-loopback noth notificat offlin ollama onc opencod operat opin opt-in orb orchestrator order os otherwi outcom outsid over-stuf overview overwrit owner owner-only packet panel panel-review parallel param parameter parent parity part pass past patch path pattern pau paus pause-resum payment pct per per-pha per-provider permiss persona pha pick pick-up pickup pickup-analysi pickup-review pictur pid plain plan plan-only plan-review planner plausibl plausible-but-wrong plu policy ponytail portabl positiv post post-turn postur pre pre-publish predict prefix preserv preset preset-ready press preview proc profil progress project project-param prompt propo protect prototyp prov provider provider-auth provider-nativ proxy prun publish push quality quality-arbitrat quest queu quick quickstart quot quota qwen rang rat rate-limit rather re re-run re-sequenc re-validat reach read-only read-writ read_only readonly ready real reason recent recogniz recommendat record recoverabl red redact reduc reduce-effort referenc refin refu register reject remember remot renam reorder repair repeat replac replay report repository request requir resilienc resolv respect respons restart restor resum retent retri retry retyp reu reus revalidat reversibl review review-authz review-correctness review-inject review-item review-risk review-secret review-security-risk review-test reviewer rewind right risk risky roadmap rol root round rout rul run runtim safety saga sandbox say scaffold scan scheduler schema scop scor seat second-review secret secret-scan security security-review seed selector separat sequenc sequentially servic sess setup shap shar sharpen shell shift sign sign-off simplify skill skip slower smaller snapshot soft someon someth sourc sovereignty spawn spec spec-up spec-up-intak spec-up-review spec-up-roadmap spend src sse ssrf stabl stag standard stat statu stay steer step step-by-step story strict strong stuf styl subcommand subscript suggest summary supers supervi supervis supervised-task supervisor supervisor-control surfac swap switch switcher synthesiz tab target task task-lifecycl teach team telemetry termin test themselv think thorough threshold tighten timelin timeout tip token touch tour transient tre troubleshoot trust ts turn twic two ui unattend unavailabl uncommit undo uneven unprotect unsaf unsandbox until unver upload uploader usag usd using validat var ve vendor ver verbatim verdict verificat verifier verify veto via vib vibestrat vibestrate-md vibestrate_param visibl wait waiting_for_approv walk walkthrough wall warn watch weak week welcom went whol why-a-human wider window without workflow workspac worktr worktre yaml yellow yml zero zero-egress",
  "entries": [
    {
      "id": "cli/abort",
      "kind": "cli",
    ...
    },
    {
      ...
      "titleTerms": "annotat",
      "terms": "40 40-58 58 a actually add agent an annotat as auth authoritativ bear befor bug codebas concept copy deeper did do don fil for generat going guidanc her human in is know lin load load-bear not one order pattern pin project rang read refactor resolv runtim safety see sess short simpl src start t task the them thes they thi tip to touch treat ts user visibl what when which whol word work would x you",
    ...
    },
    {
      ...
      "titleTerms": "policy",
      "terms": "12 a add adopt advanc advic advis an and at author automat avoid avoid-em-dash block both broken chang character cli concept confirm consol consult dash deeper deterministic did discard do draft em em-dash enforc engin english every eyebrow fix from gat glanc going hard hyphen id in it keep know label list log matcher merg migrat new no no-em-dash no-eyebrow not of on one only option owner owner-author owner-only past pend persona policy policy_advis policy_block preferenc project prompt propos recent reject remov review rul run s sect security see sid simpl slip soft stay stop suggest supervisor surfac test the tier tip to use vib vibestrat vs what word would writ yml you",
    ...
    },
    {
      ...
      "terms": "a abstract actually agent already an and away befor build by chang concept config crew deeper default dependency did diff do doe execut exist fals featur fewer fifty going her in insid inst is it know library lin mak nativ need not off on one only over over-build ponytail postur provenanc quest set simpl smaller smallest standard stop task that the thi tip to trad trust turn vib what why will word work writer you",
    ...
    },
    {
      ...
      "terms": "a acm acme-api and answer anyth api are ask astro bdd brand by check chosen cohesiv color concept deeper default deploy did edit env environment every explicit fail fast fastify fill flow for form framework generat gitignor given glob going hous how id in increment insid instruct is json just keep key know later list methodology my my-deploy nam need never nich not onc openai openai_api_key option otherwis palett param parameter per per-flow planner profil project project-glob project-param recogniz remov requir retyp reus rol run sam scaffold scop secret set shar simpl so stor str styl supersed tdd the them then thing tip token tru typ type-check unknown unset use valu vib vibestrat vibestrate_param vibestrate_param_color_token when word would x yml you",
    ...
    },
    {
      ...
      "titleTerms": "skill",
      "terms": "a act agent alway an api as assign at attach befor belong bit boundary branch caller cent cod codebas coerc common concept consol convent crew currency deeper default did domain endpoint ephemer error error-handl every everyth executor extern fetch fil float for go going handl help hous id idempotency idempotent in includ info inlin input inst integer is it its js json key know knowledg lik log logger look markdown mcp md messag mistak money must ndjson never no of on onc oncall oncall-runbook one payment planner post process project prompt put read real refund reject return rol rul run runbook runtim safety servic simpl skill so sourc src stor stuck that the them thi thing through tip to touch transact typ unknown use using validat vib vibestrat vs what when why word writ yml you your",
    ...
    },
    {
      ...
      "titleTerms": "run stat",
      "terms": "a abort advanc agent agent-rais all allow allowed_transit and approv architect are ask at automat between block chang cli concept creat deeper did each enforc error execut fail filter fix for gat going guidanc honest id in is it json keep kind know lifecycl list look mark matter mean merg merge-ready merge_ready mov of one only open paus plan rais ready record recoverabl reject replay request request-chang requir resum review rul run simpl stag stat statu sticky termin that the them tip to transit trust two validat verify vib wait waiting_for_approv what wher why word worth you",
    ...
    },
    {
      ...
      "terms": "20 a acceptanc add and at audit author between by checklist clear concept conductor context curat dashboard deeper did do doe driv enabl enhanc escalat fals fil fresh from glanc goe going ground halt how id in instead invariant it know ledger log max mod model new not objectiv of one only order packet pass paus plain plan plan-only profil project re re-ground re-sequenc refin relat remov reorder resum run sequenc simpl spend src statu step supervis supervised-task supervisor task the tim tip ts usd vib vs what with word writ writer yet yml you",
    ...
    },
    {
      ...
      "titleTerms": "supervisor",
      "terms": "a adopt advanc advis aim an and apply approv archetyp are at auth auto automat block bring by call can car chang cheap cheap-reviewer claud cli concept copy cross cross-model deeper default did enforc engineer every flow for gat glanc going haiku hard harden has hawk heavier honest how in into it its judgment just know label len list login look mak migrat mod model mor not of off on one opin or own payment permiss permission-mod persona pick plan policy postur prefer preferenc preset profil project provider reason record remov replac resum review reviewer risk risky rul run s sandbox sandbox-suggest seat second security security-hawk sign sign-off simpl singl single-profil six staff staff-engineer statu stop structur suggest supervisor the then thing thrifty tip to touch turn two use verdict vib vibestrat want what when who why word work would yml you",
    ...
    },
    {
      ...
      "titleTerms": "control supervisor",
      "terms": "40 6 a about act add advanc advis already an and answer api approv are as ask assum at attach attachment automat autonomy back be becom block budget build but button can cannot ceil chat checklist cli clos cod com concept config control conversat creat crew deeper deni destinat did diff differ discuss do doe don edit effort error exist fail fil fin flag from gat get glanc going hard header held how i in instruct is it item its itself know last leav let limit liv mad mak materi matter max max-turns-run md me messag miss nam never new no not noth now on onc one only or panel permiss phas preview profil project propos prov public quest quot rat rate-limit rather real reason remember reply request requir require_approv resum retry reversibl review rout run s sam second set should show simpl start statu stop stream supervisor supervisor-control switch task than that the thi think thre thread tip to todo tool turn undo uneven upload uploader verbatim vib vibestrat what who why will without word work would writ you your",
    ...
    },
    {
      ...
      "terms": "a advisory against and apply approv arbitrat architectur ask at author be bil boundary build careful codebas codebase-map command commit concept configurat constraint consult convent credenti crew critic deeper development did do domain e els execut explain express extra flow for g gat get goe going good guid guidanc head heavier how id implementer in init install is isolat it its json know known lean learn ledger lesson lint locally machin machine-own manu map md mod model money never not noth onc one only or orchestrat order other own path planner pnpm policy prefer preferenc print project propos provider quality quality-arbitrat quest rank re re-explain read regenerat review risk rol root rul run sandbox secret servic show simpl so src stay surfac task test the themselv thi through tip to today touch typecheck use validat vib vibestrat vibestrate-md what when who word writ you your",
    ...
    },
    {
      ...
      "terms": "a an and answer anywher at author built button can check concept consult control crew dashboard deep deeper did do doe down drop exampl exist first flow for generat glanc going guid has how i in into is it know link mak me mov navigat not noth on only or pag panel point policy press quest right ring run screen set show simpl someth step supervisor tak that the ther through tim tip to tour turn under vibestrat walk walkthrough when wher word work written wrong you your",
