...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "title": "vibe learn",
      "source": "CLI reference (generated from the command tree)",
      "summary": "Regenerate .vibestrate/CODEBASE.md, an auto-derived map of the project's stack, layout, and routes, and harvest the codebase's TODO markers.",
      "titleTerms": "learn",
      "terms": "an and auto auto-deriv codebas deriv harvest json layout learn map marker md of project regenerat rout s show stack the todo vib vibestrat",
      "body": "```text\nvibe learn - Regenerate .vibestrate/CODEBASE.md, an auto-derived map of the project's stack, layout, and routes, and harvest the codebase's TODO markers.\n  vibe learn show - Print the current CODEBASE.md (run `vibe learn` first if there is none).\n  vibe learn todos [--json] - Print the harvested TODO counts. `vibe todos` reviews and promotes them.\n```"
    },
    {
      "id": "cli/ledger",
      "kind": "cli",
    ...
    },
    {
      ...
      "title": "vibe todos",
      "source": "CLI reference (generated from the command tree)",
      "summary": "Review the TODO markers `vibe learn` found in your code, and promote the real ones onto the Board.",
      "titleTerms": "todo",
      "terms": "all and board cod dismiss found in json learn marker one onto priority promot real review the titl todo undismiss vib your",
      "body": "```text\nvibe todos [--all --json] - Review the TODO markers `vibe learn` found in your code, and promote the real ones onto the Board.\n  vibe todos promote [--title --priority --json] - Promote harvested TODOs onto the Board. Omit the fingerprint to pick one with TAB.\n  vibe todos dismiss - Set TODOs aside so later scans stop offering them. Reversible with `undismiss`.\n  vibe todos undismiss - Bring dismissed TODOs back into the review list.\n```"
    },
    {
      "id": "cli/ui",
      "kind": "cli",
    ...
    },
    {
      ...
      "titleTerms": "command",
      "terms": "by chang command ms profil scop timeout validat",
      "body": "```yaml\ncommands: object default: {\"validate\":[],\"validateTimeoutMs\":900000,\"scopeValidationByChange\":true,\"validationProfiles\":{}}\n  commands.validate: array<string> default: []\n  commands.validateTimeoutMs: number default: 900000\n  commands.scopeValidationByChange: boolean default: true\n  commands.validationProfiles: record<string, object> default: {}\n```"
    },
    {
      "id": "config/commits",
      "kind": "config",
    ...
    },
    {
      ...
      "terms": "0 1 127 4317 a agent an api approv architectur as assert binary by child cli cod code_writ coding-agent component control cor daemon default deliberately demand did down export fit for four from gat git glob how in invocat it know loc machin manag map miss model no nod on one only orchestrator os overview own per piec process project provider read read_only record relat remot run s sandbox see serv server shell simpl spawn src telemetry the thing tip to together transit ts typ ui v validat vib vibestrat wait waiting_for_approv what word worktre writ yml you your",
    ...
    {
      "id": "docs/architecture/schematics",
      "kind": "doc",
      "title": "Schematics",
      "source": "Vibestrate docs: architecture/schematics",
      "summary": "Every diagram in the docs on one page, in the order the pieces depend on each other.",
      "titleTerms": "schematic",
      "terms": "a and architectur default depend diagram doc doe driven each every fail flow hard hold how in is mad of on one order other pag piec profil project relat resolv run schematic simpl the tip turn typ what word yml",
      "body": "## In simple words\n\nThe drawing set. Every diagram that appears on a concept page is collected here in dependency order, so the whole system can be read in one pass rather than found a page at a time.\n\n**Tip.** Each diagram links back to the page that explains it. If a picture raises a question, that page answers it.\n\nNothing here is new: these are the same figures, on one sheet.\n\n## The types, and what each one holds\n\nNine types, in the order they depend on each other.\n\n| Type | What it holds | What it points at | |---|---|---| | Task | The intent, its checklist, its history | the runs it started | | Run | One attempt: status, branch, worktree, verdict | a Crew, a Task, a snapshot of a Flow | | Flow | The ordered recipe, its seats and its loop | its own Steps and Seats, nothing else | | Step | One phase: kind, stage, inputs, outputs | a Seat, when its kind takes one | | Seat | A label and a description. That is all | nothing. It is a slot | | Crew | Your roster, plus two overrides | its Roles | | Role | Prompt, permissions, skills, the seats it fills | a Profile | | Profile | Model, effort, token cap, timeout | a Provider | | Provider | Command, args, env, settings | the binary or endpoint on your machine |\n\nThe seam falls between Seat and Role. Everything above it is what a flow ships and can travel; everything below it is yours.\n\n## What a flow is made of\n\nFlow seats the slots steps the order Seat Step seat Crew answered by no field here can name a model, a provider or a price\n\nA flow is closed: its two references point at its own types. Explained on Flow."
    },
    {
    ...
    },
    {
      ...
      "terms": "0 1 1250 125024 127 20 20260614 20260614-1250 20260614-125024-add-audit-log 4317 4400 8 a abort accept acm add advanc advis agent allow allow-token-to-custom-host an and any api apply approv architect architectur are area arg as ask assess assign assuranc astro audit auth automat automatabl back bas base-url befor boolean bring browser bug built built-in bundl card cd chang cheap check clear cli cod codebas codebase-map com command complet config configur consult content control convent cor could count crew current custom dashboard deep deep-review default definit descript detail detect did discoverabl dismiss do doctor don draft enabl enough enter env every execut export fals fetch fil filter find fingerprint first fix fixm flow follow for framework friendlier from full gat generat get git github group guidanc h hack handl healthz heavy high host how hub i id if import in init input insid inspect inst interactiv is it its json key know l last learn leav ledger level lik lin list log login look loop machin mak map marker max md miss mor my my-flow nam need new no no-open not noun null number of offer oldest on onc one only open or out overview overwrit param parameter pass past path paus payment per plan planner pnpm port prefix preset preview print profil project project-param promot provider publish raw re re-run read read-only readabl redo referenc regenerat reject remov renam render replay request request-chang resolv resum resume-from resume-stag reus review review-heavy reviewer rewind right risk rout run s sam say scaffold scan schema semver set settabl setup shap shell show simpl sk skill slug so spec spec-up spend stag start statu steer step stop stor strong subcommand suggest supervis supervisor t tab tabl task termin test that the then thorough tip to todo token top top-level tru typecheck ui unassign undismiss uniqu unset up url usd use user validat verb verify vers vib vibestrat vibestrate_api_token vibestrate_hub_token view wait way welcom what wher wir with word work workspac worktre worth writ wrong xxx yaml yes yml you your your-github-login zod",
    ...
    },
    {
      ...
      "titleTerms": "crew",
      "terms": "0 1 127 3 4317 a add agent ai all all-loc an and another builder by cannot careful carry cast chang cheap check checklist claim concept config configur cor cover crew crew-schema deep default did draft each edit entirely every executor fast flow from hand hav implementer in init insid install it know label len list loc loop mad mark max model need never new no of offlin on one order pag pick preset print profil project propos ready ready-mad remov review rol roster run sav schema seat seat-rol second set shell show simpl src start tak task team termin the thi thorough tip to ts two ui use vib vibestrat what which why word work worker would written yml you",
    ...
    },
    {
      ...
      "titleTerms": "profil",
      "terms": "4 a about add agent allow and another api at balanc budget built built-in bundl by carry catalog cheap claud claude-balanc claude-cheap claude-max codex concept config configur cor crew cross cross-vendor d debug deeper delet did disallow duplicat e effort exist expensiv fenc for from get guard high how id implement in is issu its itself knob know label legibility level list loc m mak max model ms must n nest new no not null off one opt opu orchestrat order patch pick planner post power preset profil profile-schema project provider r reach real refus remov review rol run s sav say schema set shell simpl sit slower sourc src statu step step-profil strict strict-writer strong sub sub-agent task termin that the thi timeout tip token tool ts used vendor vib vibestrat what when wher word would writ writer x yml you",
    ...
    },
    {
      ...
      "titleTerms": "rol",
      "terms": "3 8 a accept agent all and api approv arbiter architect are as assembl assign attach balanc block builder can carry challenger cheap claud claude-balanc claude-cheap claude-cod cli cod code_writ concept config context cor crew deep default deliberately did doe each edit every exec executor field fil fixer flow from gat glob how id implementer in init instruct is it its json know label load markdown may mcp merg merge_ready mod model new not notebook on one only order outsid patch path permiss permission-mod planner post profil project prompt provider put read read_only ready refus requir require_approv resolv review review_exec reviewer rol role-field role-prompt role-schema role-skill rul run runtim schema seat server shell ship show simpl six skill src subject such tak termin that the thi thre tip to touch ts verifier vib vibestrat what which word worker writ yml you your",
    ...
    },
    {
      ...
      "titleTerms": "run",
      "terms": "0 1 127 4317 a abort act advis approv assuranc at attempt automat autonomy block branch by carry chang changes_request concept control copy cor count creat crew decis describ detail did diff do doe driven end execut fail fil fin finish fix flow for how human id in inspect is isolat it its json know liv loop machin max merg merge_ready metric miss mod nam need needs_human not of on one only order own owner pass path paus pend permiss pid re re-run read ready record replay repo request review run sam say schema shell simpl src stat state-machin statu supervisor task termin the thre through timelin tip ts ui validat verdict verificat vib view wait waiting_for_approv watch what when why with word workspac worktre would you your",
    ...
    },
    {
      ...
      "titleTerms": "supervisor",
      "terms": "0 1 127 4317 a add adopt adoptabl advis aim alway an and apply approv archetyp are artifact auto automat block bring by call can car cheap cheap-reviewer claud compact concept config context context-packet context_inject copy cross cross-model declin default did effect enabl enforc engin engineer every fail flow for from haiku hard harden hawk heavier how id in inject input it json judgment know label len list login look mak manifest migrat mod model mor never new no not null of off on one or own packet permiss permission-mod persona pick plan policy postur preferenc profil project provider record remov replac requir resum review reviewer risky rul run s safety sandbox sandbox-suggest seat security security-hawk set shell simpl singl single-profil six staff staff-engineer statu step step-id stop structur suggest supervis supervisor the then thing thrifty tier tip to tru two ui use vib vibestrat what who why word work writ yml you your",
    ...
    },
    {
      ...
      "titleTerms": "task",
      "terms": "0 1 127 4317 9 a acceptanc add additiv adjectiv adjective-noun all and append apply archiv assuranc attach audit author auto auto-pick automat backlog becom block board bold bold-lovelac break brief bring but by c cap cap-and-continu card carry chang check checklist cli command comment concept configurabl connector constraint context continu continuou cor correctness creat crew criteria csv current d dependency deriv descript detach detail did dismiss don draft dry dry-run e enhanc enough enter exist export fil flow forc from good hand handler health how human id ids import improv in in_progress includ inherit instead into is it item its jira json key know languag ledger len lib list log logger look lovelac manually matter merg merge_ready mod mov n nam need never new not noun one only open order out overrid own parent pend per per-item pick pickup pickup-review plain plain-languag plan plausibl plausible-but-wrong profil progress propos q r read read-only ready reason ref reopen return review risk roadmap roadmap-typ rout run sav say schema security security-risk see select sentenc server set shell should simpl sourc spec src start statu step step-by-step structur summary supervis tak task termin test that the their their-export then tip titl to ts typ ui use user valu vib vibestrat what when whol why will word work would writ wrong yet you",
    ...
    },
    {
      ...
      "terms": "a against and append apply arbitrat ask at author be best bil build careful codebas codebase-map command commit concept constraint consult convent credenti crew development did do effort els entry execut explain express fixm flow for generat get go goe good guid guidanc head heavier how http id implementer in init install is it json know known layout learn ledger lesson machin machine-own manu map md mod model money never not noth onc one only open or orchestrat order other own path planner pnpm point policy prefer preferenc print project propos provider quality quality-arbitrat r rank re re-explain read refresh relat review risk roadmap rol root rout rul run s sandbox scaffold servic shell should show simpl so someth src starter surfac task teach test text that the thi through tip to todo touch typecheck ui use vib vibestrat vibestrate-md what when who word work writ x you your",
    ...
    },
    {
      ...
      "summary": "The four steps of the default flow, in order, and what each one is for.",
      "titleTerms": "a of run step the workflow",
      "terms": "0 1 127 4317 a accept add agent agent-turn and approv approval-gat arbitrat architect architectur architecture-handoff array artifact back befor block bold bold-lovelac brief by carry chang changes_request claud clean command common concept context continu contract cor crew decis decision-summary deep default did different doc doe each edit error event every execut execution-handoff express fail fast find finding-resolut finding-respon fix fixer flow flow-schema for four fresh from gat ground handoff high how human human_approv id implement in init input instruct is kind know label long longer loop lovelac many max md merg merge_ready mistak n need not of on one only opt opt-in option order output pag panel panel-review pick pipelin plan plan-handoff pnpm quality quality-arbitrat read ready recip relat repeat replay request requir resolut respon respons response-turn resum resume-from resume-stag retry reus review review-turn room run run-brief runner schema seat sess set shar simpl six six-seat skill skip split src stag step summary summary-turn termin the thi thread tip to token too track tre truth ts turn typecheck typo ui validat verify vib vibestrat wait waiting_for_approv want way what when why word workflow you",
    ...
    },
    {
      ...
      "terms": "0 1 127 4317 a add agent ai already and api architectur are ask audit boundary build call can chosen cli cod com command concept config consult content context context-url copy crew dashboard deep default did doc end environment every express field fin find flow from full get getting-start git go half hav her how http http-api id in index init is it know laid leav link log model modul next nod node_modul not of off on one only out own pag per plan profil project quest quick quickstart re re-read read read-only recip record referenc remember repo review reviewer rul run saw scaffold search set shar simpl skill start statu stay steer stop task termin that the thes thi thing tip to together ui unattend url validat venv vib vibestrat walkthrough want what when wher why word work worktre writ yml you your",
    ...
    },
    {
      ...
      "terms": "a abort act afterward all and answer append append-only approv architect artifact ask at block broker budget c can cannot chang com concept cor creat current decid deep did different disk event every execut fail find finding-respon fix fixer flow for from gat happy has history hold how human human_approv id impossibl in is it json know leav lifecycl liv look loop max md merg merge_ready mov ms ndjson need no now on only order output path paus plan policy prompt r re re-run ready reject relat replayabl repli request requir respon rest result resum review reviewer run seat sent sequenc simpl stag stat statu step step-id stuck task task-lifecycl tell the through timelin timeout tip to transit unattend until validat validation-result verdict verify vib vibestrat view wait waiting_for_approv was what when wher why with word work workflow you your yourself",
    ...
    },
    {
      ...
      "terms": "20 a advanc after and api architect architectur artifact authenticat automat block bug cannot cd chang clean cli command config context contract creat crew debug debug-fail decis deep did diff do doesn drop dry dry-run earlier end event every execut exist fail figur fil find fix flow from get git how id if in inject inspect instead is json just keep know list look main md miss mor ndjson new newest next not noth of old on or orphan out outcom output over packet per per-phas permiss phas plan plu preview project provider prun re re-run ref referenc relat replay requir restart restor restore-preview resum resume-from resume-stag retent reus review rewind rul run s sam scop see seed sharpen simpl skill snapshot sourc stag start stat statu stderr stdout step step-id t task teach test the thi tighten tip to unsaf validat verificat verify vib vibestrat view violat what when wher why with without word workflow worktre y yml you",
    ...
    },
    {
      ...
      "terms": "a abort advanc and approv are at automat back block branch bring cd chang cli control d deep detail did different end for from gat git good guidanc how human human_approv id in is it know later list max miss next not on or p pau paus pause-resum pick point policy policy-gat project r refus reject remov request request-chang requir resum resume-from round run saf safely simpl someth stag stop the thre tip to up vib vibestrat vibestrate-worktre wait waiting_for_approv way what when wher word workflow worktre you your your-project",
