...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "terms": "0 1 1-9 2 3 4 5 6 7 8 9 a abort activ activity advanc alway always-on an and approv argument autocomplet automat b back bar body branch browser budget c cap cli clos command complet config consult context context-sensitiv control crew ctrl current d daily dashboard default delet did dismiss doc doctor down e edit effort end enter esc f flag flow get git h header help hi high history hom i idl ids in in-termin it its j json k key know layout liv low m main medium miss mod n nam navigat new no notif o of on only open opt or p pag palett panel paus pick prefix previou profil project prompt q queu quit r re re-run read read-only replay resum retent roadmap run s sam scroll select sensitiv sess set shell shift show simpl skill snapshot spac spend statu subcommand suggest switch tab task termin the thing tip to toggl topic trunk up usd validat valu vib vibestrat view websit what with word worktre writ you your",
    ...
    },
    {
      ...
      "titleTerms": "crew",
      "terms": "0 1 127 3 4317 a add ai all all-loc an and another builder by careful cast chang cheap check claim concept configur cor crew default did draft each edit entirely every executor fast from hand hav implementer in init insid install it know list loc mad mark model never new no of offlin on one order pag preset print profil project propos ready ready-mad remov rol roster run sav seat seat-rol second set shell show simpl tak task team termin the thorough tip to two ui use vib vibestrat which why word work worker would written yml you",
    ...
    },
    {
      ...
      "terms": "0 1 127 20 4317 a acceptanc add all and at audit author between beyond board by card check checklist cleanly clear concept conductor config context curat did dismiss do doe enabl enhanc escalat fals fil fresh from goe ground halt hint how id in instead invariant it know ledger log max mod model new not objectiv of on one only or order packet pass paus plain plan plan-only profil project re re-ground re-sequenc refin relat remov reorder resum roadmap run sequenc shell simpl spend src statu step supervis supervised-task supervisor task termin the tim tip ts ui usd vib vs what with word writ writer yet yml you",
    ...
    },
    {
      ...
      "terms": "0 1 127 2 24 4317 5 a add agent an and as at cd check ci cli cod codebas coding-agent com config connect creat curl dashboard did doctor edit empty f failur first fix flow folder for from fs g get getting-start git git-init githubusercontent gitignor guyshonshon http in init initialis insid install installat is it js json know learn least let machin machine-readabl main md model mor newer next nod npm one only open or order pag pass pnpm point policy project provider publish r raw read read-only readabl render repair repo report requirement rol rul run s saf sam scaffold set setup sh simpl skill skip sl start statu tak termin test the thi thing tip to too ui up url vers vib vibestrat view warn what word writ yml you your your-project",
    ...
    },
    {
      ...
      "terms": "20 a advanc after and api architect architectur artifact authenticat automat banner block bug cd chang clean cli command creat crew debug debug-fail decis did diff do doesn drop dry dry-run earlier end event every execut exist fail figur fil find fix flow from get git how id if in instead is json just keep know list look main md miss ndjson new newest next not noth of old or orphan out outcom output over per per-phas permiss phas plan post preview project provider prun re re-run ref referenc relat replay requir restart restor restore-preview resum resume-from resume-stag retent reus review rewind rul run s sam scop seed sharpen simpl skill snapshot sourc stag start stat statu stderr stdout step step-id t task teach test the thi tighten tip to unsaf validat verificat verify vib vibestrat what when wher why with without word workflow worktre y yml you",
