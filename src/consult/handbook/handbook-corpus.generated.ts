...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "terms": "0 1 127 4317 a accept add agent agent-turn and approv approval-gat arbitrat architect architectur architecture-handoff array artifact befor block bold bold-lovelac brief by carry chang changes_request claud clean command common concept context continu contract cor crew decis decision-summary default did doc doe each edit eight error event every execut execution-handoff express fail fast find finding-resolut finding-respon fix flow flow-schema for fresh from gat ground handoff high how human human_approv id implement in init input instruct is kind know label long loop lovelac many max md merg merge_ready mistak n need needs_human of on one only opt opt-in option order output pag panel panel-review pass pick plan plan-handoff pnpm quality quality-arbitrat rather re re-validat read ready recip relat repeat replay request requir resolut respon respons response-turn restart resum resume-from resume-stag retry reus review review-turn room run run-brief runner schema seat sess set shar simpl skill skip split src stag step summary summary-turn termin than the thi thread tip token too track tre truth ts turn typecheck typo ui validat verify vib vibestrat wait waiting_for_approv way what when why word workflow you",
