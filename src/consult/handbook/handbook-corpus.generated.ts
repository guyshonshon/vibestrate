...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "titleTerms": "api http",
      "terms": "0 1 127 200 201 24 400 401 403 404 4317 500 a abort act advic advis agent an analyz and answer api apply approv architectur as at auth authenticat authorizat bas bearer bearer-token bind body bound branch broker by canonic carry character cheap clean clos complet confirm consult contract control control-character conversat coverag creat creator crew cross cross-origin cross-sit csrf curl currency dashboard data default definit delet deny descript deterministic don draft effort endpoint english error event exist expo export expos fail fail-clos favicon fetch fil file-or-url finish finish-now first flow flow-creat flow-delet flow-fork flow-import flow-patch fork format gat get git glanc guard guid h health hex host http http-api id ids import in initializ input integrat invok is it json kind lan leg list loc localhost loopback main merg merge-ready merge-to-main messag model ndjson need network new no non non-loopback noth now of off on one only openssl option or origin our out over overview overwrit pass patch path paus persist phas policy portability post preview problem profil project pros provider rand ready reason recommendat redact refus requir require_approv resolv resolve-first risk rol rout run s scan schema seat sec sec-fetch-sit secret secret-scan server seven sigterm sit siz sourc sse ssrf stag stage-on-integration-branch start stop stor sub sub-agent supervisor target termin text the think thread tim to token token-gat tool tru turn ui unverifi unvers url user v1 validat vers vib vibestrat vibestrate_api_token was wher which with writ written yaml yml",
      "body": "`vibe ui` starts a Fastify server (default `http://127.0.0.1:4317`) that backs the dashboard. The same endpoints are a stable, scriptable contract: every dashboard action is an HTTP call, so anything the UI does, an external caller can do too. Pin `/api/v1` in scripts - it is rewritten to `/api` before routing, so both prefixes reach the same handlers. The server binds loopback and auth is off by default. Set `VIBESTRATE_API_TOKEN` and every `/api` request must then carry `Authorization: Bearer` with that token; binding a non-loopback host without a token is refused at startup.\n\n## Endpoints at a glance\n\n```\nGET  /api/v1/health\nGET  /api/v1/flows\nGET  /api/v1/flows/:flowId/export  as canonical YAML\nPOST /api/v1/flows/import          { yaml } or { url }\nPOST /api/v1/flows                 write a project flow\nPOST /api/v1/flows/draft           English -> flow draft\nPOST /api/v1/crews/draft           English -> crew draft\nGET  /api/integration/overview     merge-ready runs\nPOST /api/integration/advice       deterministic merge advice\nPOST /api/integration/analyze      optional model risk pass\nPOST /api/integration/finish       guarded merge to main\nGET  /api/supervisor/threads       list conversations\nPOST /api/supervisor/threads/:threadId/turn\n     one turn, SSE over POST. Seven event kinds:\n     message, phase, thinking, tool, answer, done, error\n```\n\n## Authentication\n\n```\n# expose on the LAN, token-gated\nVIBESTRATE_API_TOKEN=$(openssl rand -hex 24) vibe ui --host 0.0.0.0\ncurl -H \"Authorization: Bearer $VIBESTRATE_API_TOKEN\" \\\n  http://<host>:4317/api/v1/flows\n```"
    },
    {
    ...
    },
    {
      ...
      "terms": "0 1 127 2 20 4317 4400 a abort accept acm activ adopt advisory again against agent allow allow-token-to-custom-host alongsid an and any api apply approv architect architectur area arg as assess assign assuranc astro at attach audit auth auto auto-open await bas base-url befor bind boolean browser built built-in canonic cd chang cheap check ci clear cli codebas codebase-map command config configur confirm consult context control convent cor could crew current custom dashboard deep deep-refactor deep-review default definit descript detach detect different discoverabl display doctor don draft durabl each editabl enabl enter env environment every execut export fail fals fast fetch filter fix flow follow for framework friendlier from full generat get ghp git github giv group guidanc h handl heavy help high horizont host how hub i id implementat import in init input inspect inst interactiv interactively into invocat is it its json just keep key l leak learn ledger level lin list log login look loop machin machine-readabl map max md memory miss mor my my-flow nam no no-open not null number of on one one-tim only open or out overview overwrit own param parity part pass past path paus payment per pick pip plan planner pnpm port pre pre-publish preset preview print profil project project-param prompt provider publish raw read read-only readabl recent redo refactor referenc refus regenerat reject remov renam render replac replay request request-chang resolv resum resume-from resume-stag reus review review-heavy reviewer rewind right risk rol run s safety sam scaffold schema secret see selector semver set settabl setup shap shell show singl sk skill skip slug spend stag start statu step stor str straight strong structur subcommand suggest supervis supervisor t tabl task termin test the then thorough tim to token top top-level tru typecheck ui unask unassign under unset url usd use user using validat valu verb verify vers vib vibestrat vibestrate_api_token vibestrate_hub_token vibestrate_param view warn welcom what wher whol with work worktre wrong yaml yes yml you your your-github-login zod",
      "body": "The `vibe` command is how you work with Vibestrate from a terminal. Anything you can do in the dashboard you can do here, and the other way round. Add `--json` to any command that offers it for machine-readable output, and `--yes` to a command that would otherwise stop and ask. A failure comes back as a structured error: a title, an optional detail, and often a hint pointing at the next thing to try. Run `vibe --help` for the live list of commands, which is always exactly what your install has.\n\n## The core loop\n\n```\nvibe init                                  # one-time per project\nvibe doctor                                # verify env + config\nvibe run \"Your task description\"           # start a run\nvibe status                                # see active and recent runs\nvibe replay <runId>                        # inspect any past run\nvibe path <runId>                          # where the run's git worktree is (cd into it)\nvibe rename <runId> a friendlier name      # give the run a readable display name\n```\n\n## Command shape\n\n```\nvibe                       → open the interactive shell (no args)\nvibe <command>             → run a top-level command (init, run, status, ...)\nvibe <area> <verb>         → run a subcommand under an area (provider list, config set, ...)\n```"
    },
    {
      "id": "docs/cli/shell",
      "kind": "doc",
    ...
    },
    {
      ...
      "terms": "0 1 2 20 3 4a08 5 7c1 9b2d a acceptanc add after alia an and apply as at author back between block board boolean boundary brief budget chang check checklist ci ci-migrate-the-write-path-4a08 ci-update-the-settings-model-9b2d clean clear cli cod com comma comma-separat command commit conductor config dashboard decid default defin discard display don done-when drop earlier edit els enabl enhanc error escalat everyth executor exhaust exit fail far fil fix flow from get goal goe good halt handler happen heal hint holistic id idl implement in in_progress into invariant is it item its json kept key languag later leav lik list liv look loop mark max micro micro-plan migrat model mov next no not noth now null number objectiv of off off-go on onc one only or order own parity pass path paus pend plain plain-languag plan pnpm posit proc profil progress promot re re-read re-sequenc reach read reason relat remov reorder replac request rest resum return review reviewer rol rout run s saga schema scop self self-heal-exhaust separat sequenc set shap show so spend split src stat statu stay step still stop str strong summary supervis supervised-task supervisor task task-migrate-the-settings-schema-7c1 text that the them then through titl to tru ts typ typecheck uncheck up updat usd using v1 v2 vib want watch weak went what when with work worktre writ wrot you your",
      "body": "`vibe tasks` manages this project's tasks. A **supervised** task is one you break into ordered steps first, with `vibe tasks add --supervised` and `vibe tasks checklist add`. Each step carries a scoped objective, a plain-language done-when check, and optional file hints. `vibe tasks run` then works through those steps in order, in a single git worktree, committing after each one. It stops on its own when a step fails review, when the budget is reached, or when a between-steps supervisor judges the work has gone off-goal. Only a failing step's own work is discarded; every step that already committed stays on the branch. Nothing is ever auto-merged, so a finished supervised task lands as one branch for you to read.\n\nSee supervised tasks for the concept.\n\n## The shape of a run\n\n```\n  plan once\n    │\n    ▼\n  ┌─ step ───────────────────────────────────┐\n  │  micro-plan ─▶ implement ─▶ review       │\n  │                    ▲           │         │\n  │                    └─── fix ◀──┘         │\n  └──────────────┬───────────────────────────┘\n                 │ commit, then the supervisor decides:\n                 │ proceed · enhance · escalate\n                 ▼\n             next step  ...  then one holistic review\n```"
    },
    {
    ...
    },
    {
      ...
      "titleTerms": "consult",
      "terms": "a about an and answer api apply are ask assist at auth behind block browser can caveat chang claud command concept confidenc config confirm consult cost dashboard did diff do documentat doe driv effort estimat fil flow from get git glanc ground guid hand heavier her id in init is it last leav left list look md me model need new noth null one own policy post profil project propos provider quest read really refactor reject relat run s screen shell som spend src task termin the ther thi thing ts two until updat vib vibestrat week what wher why writ yml you your",
    ...
    },
    {
      ...
      "titleTerms": "crew",
      "terms": "a act add ai and are at backend balanc broader builder by c0 character cheap claud claude-balanc cod code_writ concept crew dashboard deeper default each edit editor executor fast field fil fit from glanc glob going hand happen implementer init install into is it its json label list loc look loop mad mak match max mcp model ndjson nul of on one only pag past permiss pick preset profil project prompt purpos read read_only ready ready-mad review rol role-field role-prompt role-skill run sav screen seat secret server set setup skill task than the them thorough use vib vibestrat what when whether which who worker workflow writ yml you your",
    ...
    },
    {
      ...
      "titleTerms": "flow",
      "terms": "a actually adaptiv add and arbitrat ask assist at auth availabl chang check codex codex-review com concept continu crew customiz dashboard declar deeper default definit each edit editor error every express field fil fill first flow follow for from gat glanc going good has hub id import in install instruct is it its keep kind list mak model nam need new no of off on one one-off or order other overrid own pick pin point profil project provider quality quality-arbitrat recip referenc repeat reveiw review reviewer rol routin run s seat select set show siz skip spec step step-profil that the then thi tighten unknown up vib vibestrat what when wher which with worker worth writ yml you your yourself",
    ...
    },
    {
      ...
      "titleTerms": "policy",
      "terms": "12 a activ add advic advis at author avoid avoid-em-dash block broken captur character choic cli concept confirm consult dash do draft em em-dash enforc english eyebrow fix from gat glanc hard hyphen id in it keep label list matcher merg migrat no no-em-dash no-eyebrow not one only option or owner owner-author owner-only past pend persona policy policy_advis policy_block preferenc project prompt propos reach recent reject remov review rul run s sect security slip soft stay stop suggest supervisor supervisor-propos surfac test the tier to ui use vib vibestrat vs what would yml your",
    ...
    },
    {
      ...
      "titleTerms": "profil",
      "terms": "1 4 5 a about actually add advanc agent allow and api at be budget budget_token built built-in by catalog claud claude-max cli codex codex-fast concept config configur crew deeper delet detect dial disallow duplicat effort effort_ignor exampl expensiv fast fenc from get going gpt gpt-5 guard has how id ignor implement in insid is issu its itself label legibility list low max model ms nest no not off one opt opu orchestrat outsid overlay patch per per-profil plu point post power preset profil project provider providers-catalog quick reach refus remov reusabl rol run s say set shell sonnet sourc spend statu step step-profil strict strict-writer strong sub sub-agent task that the ther thi thos timeout to token tool used vib warn writ writer yml",
    ...
    },
    {
      ...
      "terms": "a acm and answer api astro bdd body boolean brand by check chosen ci cohesiv color concept deeper default deploy each edit enum env every explicit fail fail-fast fast fill flag flow for form framework generat gitignor glob going how id increment insid instruct is json just key list methodology my my-deploy nam never nich no now number onc openai openai_api_key option or palett param parameter path per per-flow planner profil project project-glob project-param prompt recogniz remov requir reu rol run s scaffold scop secret set shar stor str supersed tdd the them then token tru tty typ type-check unknown unset use user valu vib vibestrat vibestrate_param vibestrate_param_color_token x yml your",
    ...
    },
    {
      ...
      "terms": "120 40 a act activ add advanc after again allow allowlist already and any api applicabl apply apply-only approv are ask assuranc attend audit auto auto-retri autonomy back backend backoff be befor behavior best best-effort block blocker both boundary broker budget by can cap caus ceil chang checklist checkpoint claud cli clock codex command complet composer concept confidenc config configur confinement consult container continu control count coverag creat crew cross daily dashboard day decid decis default default-allow defens deny depth descript detail did didn diff docker doctor doe dollar don dotenv dotenv-styl down downgrad downgrade-model dur edit effect effort egress env error every exec execut exhaust fail failur fallback fell fetch field fil fix flag flow for forbid friend full fully gap gat get git glob hard harden harden-read-only hiccup hold hold-merge-for-review honest how human i id in information init install is isolat it its json kind let limit list liv load los loudly match max max-time-day max-turns-run may mcp md merg merge_ready messag min miss mod model nam nativ ndjson network never no no-network-install no-secret-writ not not_applicabl not_run noth nothing-to-verify npm of off on onc one only or os out outcom own packag panel pars partially partially_verifi pass patch path pattern paus pend per permiss permission-mod pip plan policy post post-turn postur preset preview profil project propos provider provider-nativ rat rate-limit read read-only ready real reduc reduce-effort refu refus regex request requir require_approv resilienc retri retries_exhaust retry retry-after review review_miss rid rout rul run runtim s safety sandbox scor seat secret set show sign skill so soft someth spawn spend start statu step step-by-step steps_failed_tolerat stop story stream stream-json strict strict-apply-only styl subscript supervisor t task termin that the them then thi think through tim to today tolerat tool tool_us transient tru try turn two unattend unbound unbounded_unattended_run unsaf unsandbox usag usage_limit usd use validat validation_miss verdict verifi verificat verification_not_run verify veto vib vibestrat wait waiting_for_approv wall warn was what when wher with writ yaml yml you your zero",
    ...
    },
    {
      ...
      "titleTerms": "seat",
      "terms": "a agent agent-turn approv approval-gat architectur ask brief chair chang cod concept deeper descript diff empty execut fil flow flow-schema for gat going how id implement implementer input insid is keep kind label mak nam need not output plan profil provider resolv respons response-turn review review-turn rol schema seat shareabl src step summary summary-turn task task-brief the ts turn validat what which",
    ...
    },
    {
      ...
      "terms": "a acceptanc adaptiv all an and answer any approv architectur ask at befor brief build check cod command concept consult draft ecommerc edit editor explain fil find flow gap gap-check get glanc honest how id ids in intak into is it limit mini now off one or orb proc project propos quest re re-explain register reviewabl risk roadmap round run s scop simplify spec spec-up spec-up-intak start stop stor suggest synthesiz the then to turn up vagu valu vib what wher with written yml you",
    ...
    },
    {
      ...
      "terms": "20 a acceptanc add and at audit author between card check checklist cleanly concept conductor context curat dashboard do doe driv each effect enabl enhanc escalat fals featur fil for fresh from glanc goe ground halt has hint how id in invariant it its ledger log max mod model new not objectiv one only or order own packet pass paus plain plan plan-only proc profil project re re-ground re-sequenc reason refin relat remov reorder resum review run sequenc spend src statu step supervis supervised-task supervisor tak task text the ts usd vib vs what whol with writ writer yet yml you",
    ...
    },
    {
      ...
      "titleTerms": "supervisor",
      "terms": "a adopt advis aim an and apply approv archetyp are at auto block bring call can car chang cheap cheap-reviewer claud concept copy cross cross-model decid deeper default describ each enforc engineer every feed flow for from gat glanc going haiku hard harden hawk heavier honest how into is it judgment label len list login look mad mak migrat mod model mor not of off on one or pag permiss permission-mod persona pick plan policy postur prefer preferenc preset profil project provider record remov replac resum review reviewer risk risky rout rul run s sandbox sandbox-suggest seat security security-hawk set sign sign-off singl single-profil six staff staff-engineer statu stop structur suggest supervisor task the thi thrifty to turn unattend upgrad use vib vibestrat what when who work yml",
      "body": "A **supervisor** is the judgment Vibestrate brings to a run: how hard to look at the work, and how strict to be before calling it done. It does no work itself. It sets the level of scrutiny, then writes down every call it makes.\n\nThink of a building inspector. They do not pour the concrete or hang the drywall. They decide how hard to look, send the risky parts back for a second opinion, and record every call so you can trust the sign-off.\n\nOne word covers two things in this product, so it is worth separating them up front. **This page is the setting.** `.vibestrate/project.yml` calls it a `persona`, you pick one per run, and it shapes how the work is reviewed. The **Supervisor** panel in the dashboard is something else: a *conversation* with your project, covered by Supervisor Control. They share a name and a command. `vibe supervisor list`, `archetypes`, `adopt`, `default` and `remove` manage the setting on this page; `stop`, `resume` and `status` belong to the conversation.\n\nA supervisor is advisory, and the product says so out loud. Its choices only ever add scrutiny, never remove it, and its record labels each entry as its own judgment or as a gate that fired.\n\nOne of its settings is worth knowing before the rest. `reviewerProfile` sends every review seat to a model you pick, so the code's author does not have to be its only reviewer.\n\n```\n   task ──▶ supervisor ──▶ flow   (upgraded when risky)\n                 │\n                 ├─▶ lenses    what reviewers aim at\n                 ├─▶ posture   sandboxed? approve each change?\n                 └─▶ feed      every call, labelled\n```"
    },
    {
    ...
    },
    {
      ...
      "titleTerms": "task",
      "terms": "1 1-bas a abort accessibility add adjectiv adjective-noun and append apply assuranc at audit authz back back-to-back bas between block bold bold-lovelac break brief but cap cap-and-continu chang check checklist concept configurabl consistency constraint context continu continuou correctness cost deriv detach don end endpoint enhanc enough every exist fail flow for four from glanc good hand handler happen health human ia id improv in in_progress includ inherit inject into is item json key languag len lib list log logger look lovelac mark merg merge_ready mov nam need never noun of one only open out outcom parent paus pend per per-item performanc pick pickup pickup-review plain plain-languag plausibl plausible-but-wrong practic print progress propos put read read-only ready relat reorder return review risk rout run sav secret security security-risk sentenc server set should skill src stabl start stat statu step structur submit surfac task test the tip titl to ts up use user ux ux-ia valu vib vibestrat visu visual-consistency vs weak what when whol wrong yardstick you",
    ...
    },
    {
      ...
      "summary": "Show me how\" turns an answer into a guided tour - it moves you to the right screen and rings the control it is talking about.",
      "titleTerms": "walkthrough",
      "terms": "a about an and answer anywher ask at author built button check concept consult control crew dashboard deep do doe down drop exampl exist first flow for generat glanc guid has how i in into is it link mak me mov not noth on or pag panel point policy press quest right ring run screen set show someth step supervisor tak talk that the ther through tim to tour turn under vibestrat walk walkthrough when wher work written wrong you your",
    ...
    },
    {
      ...
      "titleTerms": "worktre",
      "terms": "a abort after along are auto bar block bold bold-lovelac branch bring cd checkout concept copy d deeper default dir doe env environment every fail fil for git going id in its keep link liv lovelac main merg merge_ready modul never nod node_modul of off pem prefix project quiet quiet-tur ready real remov run runtim saf safety separat so the thi tool touch tur venv vibestrat vibestrate-worktre wher why work worktre yml you your your-project",
    ...
    },
    {
      ...
      "terms": "11434 4 5 6 a add add-provider agent already and anthropic anthropic-api anthropic_api_key api apply arg assign at bas binary can chang claud claude-cod claude-experiment claude-fast claude-sonnet-4-6 cli cod color com command common confirm crew custom declar deeper default diff dir directory do doe doesn env expect experiment extend fast fil flag going how http http-api id in input instead is it its json key know list loc localhost localhost-proxy mistak model my my-coding-cli my-model my-model-default no no-color of ollama ollama-loc on one one-shot only openai or own p per per-provider permiss pick point profil project prompt prompt-on-stdin provider proxy put qwen3 read read_only referenc register report reviewer rol run seat send server shot sonnet stdin t tak tell test the to touch typ url usag verify vib vibestrat what with work worktre wrap yml",
    ...
    },
    {
      ...
      "terms": "a abort about add adjectiv adjective-noun and anyth approv architectur artifact at big block bold bold-lovelac branch cd chang clean creat decis diff doesn don event fail fin finish first first-run fix flow for from get getting-start gh git giv go handler happen idea implement it ll log look lovelac main md merg merge_ready ndjson never next noun one or output pass pick plan pr ready ready-to-merg request revalidat review right run sav scop see set small sourc start statu structur t task the to too ui use user validat verificat verify vib vibestrat vibestrate-worktre watch well well-scop what when worktre you",
    ...
    },
    {
      ...
      "terms": "2 a add agent an and are assign auth auth-convent balanc befor chang claud claude-balanc codebas convent cooky creat crew deeper default don enrollment error error-handl fa fil flat folder for from get getting-start go going hand handl http id inlin is it json know lax list lucia mcp md middlewar nam new not one only or permiss planner profil project prompt read read_only requir review rol rout rul run s sam seat security security-review server sess short show sit skill skip so src start t thi thos to touch ts unassign under use vib vibestrat when worth writ yml you your",
    ...
    },
    {
      ...
      "terms": "a abort act an and answer append append-only approv architect architectur artifact ask at block brok broker budget can chang changes_request cod com command creat current decid deeper diff disk drop each error event every execut exit fail find finding-respon fix fixer flow for gat going happy has hold how human human_approv id implement is it its json kept leav lifecycl loop max md merg merge_ready mid mid-run mov ndjson need on one only or output pass path paus plan policy prompt provider ran re re-run read ready reject repli request request-chang requir respon rest result resum revalidat review reviewer run s say see sent stag stat statu step step-id stop task task-lifecycl the through to transit until validat validation-result verdict verificat verify vib vibestrat wait waiting_for_approv was what when wher with workflow worktre you your yourself",
    ...
    },
    {
      ...
      "terms": "a abort actually add after again ai aider aider-install an and answer anthropic anthropic-ai anthropic_api_key api approv arriv artifact at bashrc befor behind bin blank block branch but catalog cd chang check claud claude-cod clean cli cod codex com command commit concret config confirm consult could creat curl cwd d dashboard detect detected-needs-setup didn doctor doe effort effort_ignor fail fals finish fix flag flow for found fs g gateway gemini gemini-cli get git googl got guidanc has hav high hit http i id ignor in init initi insid inst install instead is is-inside-work-tre issu it key left level list login low m main max md medium minim miss need never next no not noth notificat npm of ollama openai openai_api_key operat or output panel pars pass path paus peopl pip plu pnpm policy power prefix project provider push python ready real reason reject remov repository request request-chang requir resum rev rev-pars right run say set setup sh sl stag stall start stash statu step step-id stop stuck supervisor t tab test that the them then to tre troubleshoot tru typecheck ui uncommit unexpect up validat vers vib vibestrat vibestrate-worktre wait waiting_for_approv walk was which with wizard won work worktre xhigh yml your your-project yourself zshrc",
    ...
    },
    {
      ...
      "terms": "1 2 25 3 4 40 5 50 a abort add advis advisor alongsid analyz and arbitrat artifact audit away behind block branch by can cd chang checkout config creat create-and-run dashboard deeper default different don every fail ff ff-only fil finish finish-now first flow for fram from gh git go good handler heavier her human id if inspect inspector integrat is it json just lib list log logger main md merg merge_ready model need now oldest on one only or output path pr preserv profil protect push quality quality-arbitrat re read read-only ready relat remov replay resolv resolve-first result review rout run sav server set shar sourc src stag stage-on-integration-branch start statu suggest task termin than the thi thing to touch tru ts ui vib vibestrat vibestrate-worktre walk want watch weak when workflow worktre you yourself",
    ...
    },
    {
      ...
      "terms": "a act and append append-only artifact as bold bold-lovelac broker chang cod command cost creat current dashboard data decis deni detail disk durat each event every execut exit fil flow follow for happen id if inspect inspect-progress is it its jq json liv log lovelac main md merg merge_ready messag metric n ndjson new on only open output participant past per per-command plan profil progress prompt provider r raw read ready relat replay report resolv respons result review rol run runtim runtime-metric s sav seat select sent snapshot sourc stat statu stderr stdout step step-id stream termin that the thi timelin timestamp to token transit txt typ ui validat validation-result verdict verificat verify vib vibestrat watch what wher which workflow",
