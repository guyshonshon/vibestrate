...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "terms": "20 a advanc after and api architect architectur artifact authenticat automat block bug cd chang clean cli command config creat crew debug debug-fail decis deep did diff do doesn drop dry dry-run earlier end event every execut exist fail figur fil find fix flow from get git how id if in inspect instead is json just keep know list look main md miss mor ndjson new newest next not noth of old on or orphan out outcom output over per per-phas permiss phas plan plu preview project provider prun re re-run ref referenc relat replay requir restart restor restore-preview resum resume-from resume-stag retent reus review rewind rul run s sam scop see seed sharpen simpl skill snapshot sourc stag start stat statu stderr stdout step step-id t task teach test the thi tighten tip to unsaf validat verificat verify vib vibestrat view what when wher why with without word workflow worktre y yml you",
