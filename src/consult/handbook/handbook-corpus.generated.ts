...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "terms": "2 24 5 add agent already and artifact at attachment cd check cli cod codebas coding-agent com command compos context crew curl deeper did doctor domain els empty environment event every extra fil find fix flow fs g get getting-start git githubusercontent gitignor going guyshonshon hav her hold http in init insid install installat instruct is its js json know least main markdown md metric newer next nod noth npm on one onto option or per pnpm policy profil project provider publish raw read requirement rol rul run s scaffold set sh simpl skill sl start stat that the tip touch turn two until up url vers vib vibestrat view what wir word writ yml you your your-project",
    ...
    },
    {
      ...
      "terms": "0 0a1b2c3d4 1 10 13 2 22 227 24 3 318m 4 4318 497m 5 7 9 a abort about accept acm acme-api add adjust advic advis advisor agent ahead ai aider aider-install all already amount an and anthropic anthropic-ai anthropic_api_key api appli applicabl apply approv arbitrat architect arg artifact as assuranc at auto auto-push availabl back bar bashrc be befor behind bin block bold bold-lovelac branch build but by can carry cd chang check checkout claud claude-cod clean cleanly cli cloud cod codebas codex command commit confidenc config configur confirm connect consum copy cover crew current custom dashboard decis deeper default detect did diff different disabl do doctor doe durat earn ebadengin echo edit els endpoint engin engineer everyth exactly exampl exec execut executor exist exit export express fail fast fast-forward fil fin finish finish-now first fix fixer flag flaky flow for fork forward found from g gemini gemini-cli get getting-start git git-init gitignor going googl handler happen hav head healthz help her high hold hom how http http-api id if improv in init initializ input insid install integrat into invocat invok is isn isolat it its js json just keep key know land later learn left lik lin list llm lm loc localhost localhost-proxy log login look lovelac m magic main manager master may md merg merge-to-main merge_ready mod model mov must nam need newer next no no-op nod not noth now npm nvm of often ok ollama on one only onto op open openai openai_api_key opencod operat or outcom outsid own p packag pag panel panel-review part pass path permiss permission-mod permit persona pick pid pip plac plan plan-only planner pnpm point policy port prefix present preset project prompt prompt-flag provider proxy push python quality quality-arbitrat quickstart qwen3 ran re re-test read read-only readm ready real refere repo repository requir resolv respond restart review reviewer right rol rul run s saf sandbox sav say scaffold scop script security security-review select send sentenc server set setup shap should show simpl sit skill small so someth staff staff-engineer stag stand start starter statu stay stdin stdout strict structur studio substitut supervisor t tak task tell test that the their then thi tiny tip to token took touch tri tru trunk typ typecheck ui ui-port unattend uncommit unsupport usag use v v0 v2 v22 valid validat verificat verifier verify vers via vib vibestrat vibestrate-worktre vibestrate_provider_ok view wait warn was watch wer what which whos will with without word workspac workspace-writ worktre would writ yes yml you your your-project your-write-flag yourself zshrc",
    ...
    },
    {
      ...
      "terms": "after and app are c cd claud cmd cor deeper did diff do docker doctor except fix g get getting-start going her in in-app init inst install is isolat know loop merg natively next no not npm on one or path power project provider ps1 recogniz right run shell simpl start tab termin the thing tip to vers vib vibestrat what window with word work worktre wsl you your your-project",
