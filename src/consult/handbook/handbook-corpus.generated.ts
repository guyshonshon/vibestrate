...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  "schemaVersion": 1,
  "lexicon": "110 12 120 23 24 322 40 40-58 512 58 abort absenc acceptanc access act activity adaptiv add-flow add-provider add-skill advanc advi advisor agent aider aim allow allowlist already alway amp analysi analyz analyze-risk analyze-test anchor annotat answer anti anti-pattern anyth anywher api apply apply-only approv arbiter arbitrat architect architectur archiv arg array artifact ask assign assist assum assuranc attach attachment attend audit auth authenticat author authz auto auto-retri automat autonomy availabl backend backoff banner bas becom behind best-effort big big-pictur block blocker board bound boundary branch brand brief broader broker budget builder built built-in bundl button c0 cannot canva cap cap-and-continu capability car catalog cau caveat cd ceil challeng challenge-respons challenger cheap checklist checkout claud claude-cod clean cli clo clock cloud cmd co code_writ codeba codebas codex coding-agent com command commit complet concept concurrency concurrent conductor confidenc config configur configurabl configurat confinement confirm conflict constraint consult container context continu continuou contract control control-character conversat copy correctness cost create-and-run creator credenti crew cross crush csrf ctrl ctrl-k curat cursor custom customiz daemon daily dashboard debug-fail deci decid decis decision-summary decomposit dedicat deep deeper default-allow definit delay deliberately deni deny depend deriv derived-flow destinat detach deterministic diff differ directory-map disposabl doc docker doctor documentat doesn dollar downgrad downgrade-model draft drop dry dry-run duplicat earlier early editor effect effort egress email enabl end enforc enforcement engin enhanc enter env environment ephemer esc escalat estimat event everyth evidenc execut executor exhaust exist exit expect explicit export express extend extern fail-clo fallback fast featur fell fetch field fifty fil filesystem fill fin first first-run fixer flag flaky flight flow forbid form fresh full gap gap-fill gat gateway gemini generat getting-start git git-tree-merg glanc glob glossary good goos got guard guid halt hand handoff hard harden head headless heavier held hero high hint hold hom honest horizont http http-api human id ids imag implement implementat implementation-review implementer improv in-progress index info informat inherit init initializ inject input insid inspect inspect-progress inspector inst installat instead instruct intak integrat interactiv invariant isn isolat item itself json judgment key kind label last leak learn least leav ledger legibility len library lifecycl limit link liter liv loader loc localhost localhost-proxy log look loop loopback loudly machin mad main map markdown matcher materi matter max mcp md ment merg merge_ready messag methodology metric micro micro-plan min minimalism mod model mov ms narrow nativ navigator ndjson network never newer next non non-cli non-loopback noth notificat nul objectiv ollama onc opencod operat opt-in orb orchestrator os outcom outsid over-stuf overview overwrit owner owner-only packet panel panel-review param parameter parent parity pass past patch path pattern pau paus pause-resum pct per per-pha per-provider permiss persona pha pick pick-up pickup pickup-analysi pickup-review pictur pid plain plan plan-only plan-review planner plausibl plausible-but-wrong plu point policy ponytail positiv post post-turn postur pre pre-publish predict prefix preserv preset preset-ready press preview proc profil progress project project-param prompt propo protect prov provider provider-auth provider-nativ proxy prun publish pull push quality quality-arbitrat quest queu quick quickstart quot quota qwen rang rat rate-limit rather re re-run re-sequenc re-validat reach read-only read-writ read_only readonly ready real reason recent recogniz recommendat red redact reduc reduce-effort referenc refin refu register reject remot renam reorder repair replac replay report repository request requir resilienc resolv respect respons restart restor resum retent retri retry reu reus revalidat reversibl review review-authz review-correctness review-inject review-item review-risk review-secret review-security-risk review-test reviewer rewind right risk risky roadmap rol root round rout rul run runtim safety saga sandbox scaffold scan scheduler schema scop scor seat second-review secret secret-scan security security-review seed selector sequenc sequentially servic sess setup sharpen shell shift sign sign-off simplify skill skip snapshot soft someth sourc sovereignty spawn spec spec-up spec-up-intak spec-up-review spec-up-roadmap spend src sse ssrf stabl stag standard stat statu stay steer step step-by-step story strict stuf subcommand subscript suggest summary supers supervi supervis supervised-task supervisor supervisor-control surfac switch switcher synthesiz tab target task task-lifecycl teach telemetry termin test think thorough threshold tier tighten timelin timeout token touch tour transient tre troubleshoot ts turn twic ui unattend unavailabl uncommit undo uneven unprotect unsaf unsandbox until unver upload usag usd using validat var ve ver verbatim verdict verificat verifier verify veto via vib vibestrat vibestrate-md vibestrate_param visibl vs wait waiting_for_approv walk walkthrough wall warn watch weak week welcom whol why-a-human wider window without workflow workspac worktr worktre yaml yellow yml zero zero-egress",
  "entries": [
    {
      "id": "cli/abort",
      "kind": "cli",
    ...
    },
    {
      ...
      "summary": "Build a flow around the task instead of picking one off the shelf - and why the model never authors the graph.",
      "titleTerms": "deriv flow",
      "terms": "110 120 2 23 322 a absenc ambiguou and answer api are around at auth author b block build c cannot concept concurrency data data-integrity decomposit depend deriv derived-flow describ design do doe down evidenc flow flow-forc forc fre glanc graph input instead integrity is it itself len mad max max-unit migrat model money n need never not noth of off on onc one only performanc pick positiv public public-api requir review rol scor seat seat-rol second secret shelf skip stand static step task test that the ui unit untrust untrusted-input we what when why work workflow writ",
      "body": "A Flow is a generic layer, so a fixed recipe is always slightly wrong: too heavy for a one-line change, too light for a migration that touches money. Deriving a Flow builds one from the work itself.\n\nGive it a task like *\"Add team billing: (a) a teams table, (b) an owner-only members endpoint, (c) a monthly invoice endpoint. c depends on b, b on a. End-to-end billing tests only run once all three are in.\"* and it compiles the graph those sentences describe.\n\n## The model describes the work. It does not design the workflow.\n\nA shaping turn returns a **decomposition**, not a flow: the units of work, what each one depends on, and what each one is risky about. Deterministic code turns that into the graph.\n\nThat split is the safety property, and it is the same reasoning that makes a `block` policy owner-only. Every step id, every `needs` edge and every review lens is computed, so a model can influence *what* gets reviewed only through a closed vocabulary, and can never remove a gate, reorder one, or write a step that renders its own verdict. A poor decomposition produces a flow that is wrong-but-gated, never one that is ungated.\n\n## What it answers that a static flow cannot\n\n**\"c depends on b.\"** Each unit becomes its own implement step, wired to the units it named. Unit steps are also chained in dependency order even when two units are independent - they share one git worktree and both write the diff, so running them at once would race.\n\n**\"Testing only once a, b and c are made.\"** Validation waits for every unit step. A partial tree cannot be meaningfully validated."
    },
    {
      ...
      "titleTerms": "flow",
      "terms": "a adaptiv add advanc and arbitrat as ask assist at auth automat check choos clear cli codex codex-review com concept contain continu crew customiz deeper default definit deriv draft each edit error every express field fil first flow follow for from glanc going has hub import install instruct it its keep kind list mak model nam need new no no-select of off on one open or order other overrid own pick point profil project provider pull quality quality-arbitrat recip referenc reveiw review reviewer rol run s sav select set show siz skip spec step step-profil the then thi tighten unknown up use vib vibestrat what when worker writ yml you your",
      "body": "A **Flow** is a recipe: the ordered steps a run works through, and the *kind* of worker each step needs. It names seats, so one step calls for an implementer and the next calls for a reviewer, and it never names an AI model. Your Crew supplies the workers. That is what lets you run a Flow someone else wrote with your own models and your own budget.\n\nThe **Flows** page is where they live: one card per Flow this project can run.\n\n!The Flows page. Each flow is a card with a bar of its steps colour-coded by kind, counts of its steps and seats, and an Open button. The header reads Flows, with New flow and Import buttons, a project-owned count, and a legend naming the four step colours: Build, Review, Check and Gate. A Draft a flow panel sits below the header.\n\nThe bar on a card is that Flow's steps in order, coloured by the job each one does, and the legend names the four: Build writes the change, Review judges it, Check runs commands that pass or fail, Gate stops for a person. So a card gives you a Flow's length and its makeup before you open it, and the counts beside the bar say it in numbers. Nothing on a card names a model, because no Flow step has one.\n\n**Open** takes a Flow into the Flow Builder, which lists every step with its seat. Built-ins load read-only.\n\nVibestrate ships the `default` flow: plan, architecture, implement, validate, review, verify, with fix and re-validate looping in when review asks for changes. Workflow is the canonical description of it, step by step."
    },
    {
      ...
      "titleTerms": "policy",
      "terms": "12 a activ add adopt advanc advic advis an and at author automat avoid avoid-em-dash block both broken chang character cli concept confirm consult dash deterministic discard do draft em em-dash enforc engin english eyebrow fix from gat glanc guard hard hyphen id in it keep label list matcher merg migrat new no no-em-dash no-eyebrow not of one only option owner owner-author owner-only past pend persona policy policy_advis policy_block preferenc project prompt propos recent reject remov review rul run s sect security see sid slip soft stay stop suggest supervisor surfac test the tier to use vib vibestrat vs what would writ yml",
      "body": "A **policy** is a rule the project enforces on every run. Policies belong to the *project*, not to one supervisor - so a rule like \"use a hyphen, not an em-dash\" holds no matter which supervisor reviews the work. The active supervisor is the *enforcer*: it carries them into the review, but it does not own them.\n\nThey live on the **Policies** page in the sidebar.\n\n!The Policies page. The header reads Policies over a New policy button, a 4/4 Guards on tile, and counters reading 2 advise, 0 block, 0 pending and 0 engine rules. The left column holds Your policies with two rules tagged advise, over a Deterministic engine card reading No rules in .vibestrate/policies/*.yml. The right column lists the Hard guards toggles - Forbid main-branch writes, Forbid secrets access, Forbid auto-push, Forbid auto-merge - each switched on.\n\nThe header counts what the project carries: advise, block, pending, and engine rules. This project holds two, both at the advise tier, so the reviewer is told about them on every run. The four **Hard guards** on the right ship on for every project; the left column is the part you author, and a fresh project starts with it empty.\n\nEach policy has a **tier** that decides how it is enforced:\n\n**advise** - the default. The rule goes to both sides of the change: the seat writing the code is told to comply with it, and the reviewer is told to check the diff against it. A violation that gets written anyway is flagged and rides the normal review and fix loop. Right for anything a human judges: a model generalizes to paraphrases a brittle pattern would miss. An advise rule never blocks a merge on its own.\n\n**block** - a regex matched against the lines the run **added**, not a model verdict. On a match the run lands `blocked` with the reason shown, even if th\n…"
    },
    {
    ...
    },
    {
      ...
      "titleTerms": "profil",
      "terms": "1 4 5 a about actually add advanc agent allow and api at automat be budget budget_token built built-in by catalog cheap claud claude-cheap claude-max cli codex codex-fast concept config configur crew deeper delet detect dial disallow duplicat effort expensiv fast fenc from get going gpt gpt-5 guard has high how id implement in insid is issu its itself label legibility list low max model ms nest new no not off one opt opu orchestrat outsid overlay patch per per-profil plu point post power preset profil project provider providers-catalog reach real refus remov reusabl rol run s say set shell sonnet sourc spend statu step step-profil strict strict-writer strong sub sub-agent task that the ther thi timeout to token tool ui used vib vibestrat writ writer yml",
      "body": "A **Profile** decides how strong and expensive a Role runs. It is a saved preset that bundles a **Provider** (where the work happens), the **model**, and the **effort** level, so a Role can point at it instead of naming a model itself.\n\nYou keep them on the Profiles page. `vibe ui` opens the dashboard on localhost, and Profiles sits in the sidebar.\n\n!The Profiles page in the dashboard, open from Profiles in the sidebar. The header counts one profile across one provider, with New profile at the top right. Under a claude heading, the claude-balanced card is marked used by 6 roles and shows provider, model and effort as tiles above fields for Provider, Label, Model, Max tokens and Timeout (ms), an Effort scale from Faster to Smarter with medium selected, and Duplicate, Delete and Saved along the bottom.\n\nEach card is one Profile, filed under the provider it runs on. The tiles say what it resolves to today, the fields under them are where you change it, and the count beside the name is the Crew roles pointing at it. Edit one and every one of those roles runs on the new setting, from the next run.\n\n**A Role points at a Profile, not a model.** A Role names a Profile by its id, and the Profile holds the actual provider, model, and effort. So you swap the model for every Role on a Profile by editing one place, and a Role never hard-codes a model itself.\n\nThink of it like the drive modes on a car. \"Eco\" and \"Sport\" don't change who is driving, they change how hard the engine works. A Profile is that setting for an AI worker, saved with a name so you can reuse it.\n\nA Profile sets five things: the `provider`, the `model` id, the effort level (`power`), an optional per-turn output cap (`maxTokens`) and a per-turn `timeoutMs`. **New profile** in the header adds one; **Duplicate*\n…"
    },
    {
    ...
    },
    {
      ...
      "summary": "The dashboard shows you every gate a run passed. Underneath, the Action Broker decides each real effect against your rules and writes down what it decided.",
      "titleTerms": "act broker policy safety",
      "terms": "120 40 a abort act activ advanc after again against allow allowlist already anchor and any api applicabl apply apply-only approv are artifact as ask assuranc at attend audit auto auto-retri automat autonomy back backend backoff banner be befor behavior best best-effort block blocker both boundary broker budget by can cap caus ceil chang check claud clear cli clock codex command complet composer concept confidenc config configur confinement container continu control count coverag creat crew cross daily dashboard day decid default default-allow defens deny depth descript detail deterministic did didn diff docker doctor doe dollar don dotenv down downgrad downgrade-model duplicat dur each early edit effect effort egress engin env error every exec execut exhaust exit fail failur fallback fell fetch field fil fix flag flow for full fully gap gat get git glob guard hard harden harden-read-only hiccup hold hold-merge-for-review i id in information init inspect install instead is isolat it its json kind lik limit list load loader los loudly match max max-time-day max-turns-run may mcp md merg merge_ready messag min miss mod model nam nativ ndjson need network never no no-network-install no-secret-writ non non-zero not not_applicabl noth nothing-to-verify npm of off on onc only or os out own pars partially partially_verifi pass past patch path pattern paus per permiss permission-mod pip plan policy post post-turn postur preset profil project provider provider-nativ rat rate-limit re re-run read read-only ready real reduc reduce-effort refu refus regex reject render repair report request requir require_approv resilienc retri retries_exhaust retry retry-after review review_miss rid root rul run runtim s safety sam sandbox scor seat secret set show sign skill so soft someth spawn spend start statu step step-by-step steps_failed_tolerat stop story stream stream-json strict strict-apply-only subscript supervisor t tab termin that the them then thi think tim to today toggl tolerat tool tool_us transient tre tru try turn two unattend unbound unbounded_unattended_run underneath unsaf unsandbox usag usage_limit usd use validat validation_miss verdict verifi verificat verification_not_run verify veto vib vibestrat view wait waiting_for_approv wall warn what wher with writ yaml yml you your zero",
      "body": "Open any run and the safety model is the first thing on the page. The **Run assurance** panel carries a single verdict and four lanes under it - policy, validation, review, verification - so you can see which gate held and which one stopped the work.\n\n!A blocked run on the Express flow. The Run assurance panel reads blocked, with review showing changes requested and verification showing failed. Below it the reviewer's finding says the new code writes userId to stdout on every rejected save. The panel's controls are View review and Re-run with fixes, and Pause and Abort sit in the run header.\n\nThis run got stopped by its own gates. Review came back **changes requested**, verification failed after it, and the verdict is blocked, so nothing reached your branch. **View review** opens the finding in full, and **Re-run with fixes** sends the work back with that finding attached instead of starting over. **Pause** and **Abort** are your two levers while a run is still moving.\n\nThe panel is a summary. Each of those lanes is fed by decisions made one at a time, while the run was going, by the **Action Broker**."
    },
    {
    ...
    },
    {
      ...
      "titleTerms": "seat",
      "terms": "a advanc agent agent-turn and approv approval-gat architectur automat brief chair cli concept cover crew deeper default descript diff empty execut fil fill flow flow-schema gat going id implement implementer input insid is its keep kind label nam need not order output plan profil respons response-turn review review-turn rol s schema seat shareabl show src step summary summary-turn task task-brief that the their them they ts turn validat vib what whether which your",
      "body": "A **Seat** is an empty, labelled chair in a Flow that says \"this step needs someone to fill it.\" It is a contract, not a person: it names the *kind* of worker a step needs, and nothing about who.\n\nThe Crew page is where the chairs meet the people who sit in them.\n\n!The Crew page. Each role is listed as a row naming the Seats it can fill and the Profile it runs on, under a Crew header.\n\nEvery row is one of your Roles. The Seats column is the set of chairs that Role will take, and the Profile column is the model it brings when it takes one. The page also counts the seats your Flows can ask for and flags any that no Role covers, so an unfillable run shows up before you start it.\n\nA Flow sets out the chairs and never says who sits down; your Crew does that, at the moment a task runs. That gap is the whole point: a Flow names chairs and never names your AI models, so you can take a Flow someone else wrote and run it with your own workers.\n\n## Advanced: CLI and automation\n\n```\n# a Flow's seats, its ordered steps, and whether your Crew covers them\nvibe flows show default\n\n# your Crew's roles, their profiles, and the seats they fill\nvibe crew show\n```\n\n```\nseats:\n  implementer:\n    label: Implementer\n    description: Implements the plan and architecture.\n\nsteps:\n  - id: implement\n    label: Implement\n    kind: agent-turn\n    seat: implementer\n    inputs: [task-brief, plan, architecture]\n    outputs: [execution, diff]\n```"
    },
    {
    ...
    },
    {
      ...
      "terms": "a acceptanc adaptiv all an and answer any approv architectur at befor brief build cod command concept consult ecommerc edit editor fil find flow foo gap get glanc honest how id ids in intak into is it limit mini no no-select off orb proc project propos quest register reviewabl risk roadmap round run s scop select simplify spec spec-up spec-up-intak start stor suggest the to ts turn up vagu valu vib what wher written yml you",
    ...
    },
    {
      ...
      "titleTerms": "run stat",
      "terms": "a abort advanc agent agent-rais all allow allowed_transit and approv architect are ask at automat between block chang cli concept creat deeper each enforc error execut fail fix for gat going guidanc honest id in is it json keep kind lifecycl list mark matter mean merg merge-ready merge_ready mov of one only open paus plan rais ready reject replay request request-chang requir resum review rul run stag stat statu sticky termin that the them transit two validat verify vib vibestrat wait waiting_for_approv what why you",
      "body": "Run state is the Status column on the dashboard's **All runs** page. Every run this project has recorded gets a row there, and that column carries the one value the run is at right now.\n\n!The All runs page in the dashboard. Four runs sit in a table with Task, Status, Review, Verify and Duration columns, one row reading merge-ready. Above it are a Filter by task or id box, a Prune snapshots control and a Scheduler strip offering Start the queue and Open the board.\n\nA run holds one status at a time. Think of a package you've shipped: it's in one definite place, \"out for delivery\" or \"delivered\", never two at once and never somewhere the tracking made up.\n\nThe column reads a saved value. It lives in a `state.json` file under `.vibestrate/runs/`, in the folder named after the run id. The `status` comes from a fixed set of sixteen values, and Vibestrate validates it before writing it down. The counts above the table roll those sixteen into total, active, merge-ready and failed.\n\nA run starts at `created` and ends in one of four terminal statuses:\n\ncreated planning, architecting, executing validating, reviewing, fixing, verifying merge_ready blocked failed aborted no way back out\n\nAlong the way it can sit at `waiting_for_approval` (a gate is holding it) or `paused` (you asked it to stop).\n\n## Advanced: CLI and automation\n\n```\nvibe status\nvibe status --json\nvibe replay <runId>\nvibe pause <runId>\nvibe resume <runId>\n```"
    },
    {
    ...
    },
    {
      ...
      "summary": "The Supervisor chat panel on Mission Control. It remembers the conversation, answers from your real project, and - once the switch reads Answers and acts - turns what you say into a task or a run.",
      "titleTerms": "control supervisor",
      "terms": "40 6 a about act add advanc advis already an and answer api approv are as ask assum at attach attachment automat autonomy back be becom budget build but button can cannot ceil chat checklist cli clos cod com concept config control conversat creat crew deni destinat did diff differ discuss do doe don edit effort error exist fail fil fin flag from gat glanc hard header held how in instruct into is it item its itself last leav let limit liv mad materi matter max max-turns-run md me messag miss nam never new no not noth now on onc one only or panel permiss phas preview profil project propos prov public quest quot rat rate-limit rather read real reason remember reply request requir require_approv resum reversibl review rout run s sam say second set should show start statu stop stream supervisor supervisor-control switch task than that the thi think thre thread to todo tool turn undo uneven upload verbatim vib vibestrat what who why will without word work writ you your",
      "body": "**Supervisor Control** is the chat panel titled **Supervisor**. It fills the right column of Mission Control, and it sits on the page of any run that is still going. Type what you want. It answers from your real project, and it remembers what was said earlier in the thread.\n\n!Mission Control in the dashboard. A sidebar of sections runs down the left. The header reads Mission control, beside an approvals tile showing Clear under the caption Nothing blocked and an Edit layout button. A New run composer and a Run summary panel sit in the middle column, and the Supervisor chat fills the right column, its header switch offering Answers only and Answers and acts.\n\nThe switch in that panel header is the part to find first. It reads **Answers only** or **Answers and acts**, and it is a **permission**, not a stop: it decides whether the supervisor may make a task, add TODOs or start a run. Stop is a different control, the red square that replaces Send while a turn is running.\n\nConsult answers one question and forgets it. Supervisor Control keeps the thread, so \"do the other one instead\" has something to point at.\n\nA turn runs in three phases. **Routing** decides what you meant. **Acting** does it. **Answering** writes the reply. Routing only runs when the supervisor is allowed to act, and out of the box it is not, so a message costs one model call and changes nothing.\n\nyou routing acting answering you nothing to act on"
    },
    {
      ...
      "summary": "The plain-language brief you hand Vibestrate. You type it into the New run composer on Mission Control, and every run ends at one of four outcomes.",
      "titleTerms": "task",
      "terms": "1 1-bas a abort add adjectiv adjective-noun advanc and another append apply arbiter assuranc at audit automat back back-to-back bas between block bold bold-lovelac break brief but cap cap-and-continu chang check checklist cli composer concept configurabl constraint context continu continuou control correctness cost deriv detach don end enhanc every exist fail flow for four from giv glanc good hand handler health human id improv in in_progress includ inherit into is it item json key languag len lib list log logger look lovelac mark merg merge_ready miss mov nam need never new noun of on one only open or out outcom panel parent path paus pend per per-item pick pickup pickup-review plain plain-languag plausibl plausible-but-wrong practic print progress propos put read read-only ready relat reorder return review risk rout run sav security security-risk server set should skill src stabl stat statu step structur surfac tak task test the tip titl to ts typ ui up use user valu vib vibestrat vs weak when whol wrong yardstick you",
      "body": "A Task is what you want done, written in plain language, the way you would brief a capable colleague. You say what you want. Vibestrate works out the steps.\n\nYou write one on Mission Control. `vibe ui` opens the dashboard on localhost, and the New run composer is the first thing on it.\n\n!Mission Control. A sidebar of sections runs down the left. The header reads Mission control, with an approvals tile showing Clear under the caption Nothing blocked and an Edit layout button. A New run composer and a Run summary panel sit below, and the Supervisor chat fills the right column, its mode switch set between Answers only and Answers and acts.\n\nThe box at the top of New run is the Task, and a sentence is enough to launch. Everything under it is optional: the flow, the crew, the supervisor persona, and toggles for a read-only or unattended run. The Run summary beside it reads back what's about to start, so you see the shape of the run before you commit, and the Supervisor panel answers questions about the project while you write, in either of its two tones, Answers only or Answers and acts.\n\nYou don't list files or set an order. The Flow decides the steps and your Crew does the work. The Task is the brief.\n\nA Task becomes a *run*, and a run ends at one of four outcomes: `merge_ready`, `blocked`, `failed`, or `aborted`. It never pushes and never merges. The diff is yours to land."
    },
    {
    ...
    },
    {
      ...
      "terms": "2 24 5 add agent and artifact at attachment check cli cod coding-agent com command compos context crew curl doctor domain empty environment event every extra fil flow fs g get getting-start git githubusercontent gitignor guyshonshon her hold http in init insid install installat instruct is its js json least main markdown md metric newer next nod npm on one onto option or per pnpm policy profil project provider publish raw read requirement rol rul run s set sh skill sl start stat that turn two until up url vers vib vibestrat view what yml you your",
      "body": "You'll need **Node.js 24 or newer** and a git repository. Install the package globally, run `vibe init` inside your project, then `vibe doctor` to check your machine has what a run needs. It runs on macOS, Linux and Windows, no WSL required. On Windows the in-app terminal tab is off - see Native Windows support.\n\n## Requirements\n\n- **Node.js 24 or newer.** Check yours with `node --version`. - **git 2.5 or newer.** Vibestrate makes a second checkout of your repo for each run and tears it down after. Older git can't do that. - **npm or pnpm**, to install the package. - **At least one coding-agent CLI** on your PATH: Claude Code, Codex, Gemini, Aider, Ollama, OpenCode, or another supported provider. You can add one later, and `vibe doctor` tells you what's missing.\n\n## Install\n\n```\nnpm install -g vibestrate\n# or\npnpm add -g vibestrate\n```\n\n```\nurl=https://raw.githubusercontent.com/guyshonshon\ncurl -fsSL $url/vibestrate/main/install.sh | sh\n```\n\n```\nnpm view vibestrate versions     # what is published\nnpm install -g vibestrate@<version>\nvibe --version\n```\n\n## Set up your project\n\n```\nvibe init\nvibe doctor\n```\n\n## Inside `.vibestrate/`\n\n```\n.vibestrate/\n  project.yml  providers, profiles, crews\n               (roles), commands, policies\n  rules.md     project instructions agents\n               read on every turn\n  rules/       optional extra instruction\n               files, composed onto rules.md\n  roles/       one JSON role file per role,\n               holding its instructions\n  skills/      markdown attachments that add\n               domain context\n  flows/       your project's run Flows\n               (empty until you add one)\n  runs/        run state, artifacts, metrics,\n               events\n```\n\n```\n.vibestrate/runs/\n```"
    },
    {
    ...
    },
    {
      ...
      "terms": "0 0a1b2c3d4 1 10 13 2 22 227 24 3 318m 4 4318 497m 5 7 9 a abort about accept acm acme-api add adjust advic advis advisor agent ahead ai aider aider-install all already amount an and anthropic anthropic-ai anthropic_api_key api appli applicabl apply approv arbitrat architect arg artifact assuranc at auto auto-push availabl back bar bashrc be befor behind bin block bold bold-lovelac branch build but by can carry cd chang check checkout claud claude-cod clean cleanly cli cloud cod codebas codex command commit confidenc config configur confirm connect consum cover crew current custom dashboard decis default detect did diff different disabl do doctor doe durat earn ebadengin echo edit els engin engineer everyth exactly exampl exec execut executor exist exit export express fail fast fast-forward fil fin finish finish-now first fix fixer flag flaky flow for fork forward found from g gemini gemini-cli get getting-start git git-init gitignor going googl handler hav head help her high hold hom how http http-api id if improv in init initializ input insid install integrat into invocat invok is isn isolat it its js json keep key land later learn left lik lin list llm lm loc localhost localhost-proxy log login look lovelac m magic main manager master may md merg merge-to-main merge_ready mod model mov must nam need newer next no no-op nod not noth now npm nvm of often ok ollama on one only onto op open openai openai_api_key opencod operat or outcom outsid own p packag pag panel panel-review part pass path permiss permission-mod permit persona pick pid pip plac plan plan-only planner pnpm point policy port prefix present preset project prompt prompt-flag provider proxy push python quality quality-arbitrat quickstart qwen3 ran re re-test read read-only readm ready real repository requir resolv respond restart review reviewer right rol rul run s saf sandbox sav say scaffold scop script security security-review select send sentenc server set setup shap should show sit skill small so someth staff staff-engineer stag stand start starter statu stay stdin stdout strict structur studio substitut supervisor t tak task tell test that the their then thi tiny to token took touch tri tru trunk typ typecheck ui ui-port unattend uncommit unsupport usag use v v0 v2 v22 valid validat verificat verifier verify vers via vib vibestrat vibestrate-worktre vibestrate_provider_ok view warn was watch wer what which whos will with without workspac workspace-writ worktre would writ yes yml you your your-write-flag yourself zshrc",
      "body": "Vibestrate drives the AI coding CLIs already installed on your machine - Claude Code, Codex, Gemini, Aider, Ollama and others - as one pipeline over a single task. You type one sentence. It opens a throwaway copy of your repository, walks a plan-build-review-verify sequence there, and runs your own tests as the referee. You get a branch in that copy, waiting on your decision. Vibestrate never pushes and never merges.\n\nIf you already pay for two or three coding CLIs and use them one at a time, Vibestrate is the layer that runs them together. It removes the logistics of putting several models on one task: pasting the same context into each tool, keeping spare checkouts straight, and carrying one model's plan into the next model's prompt.\n\nThe path from an empty machine to a change on your trunk runs through eight stations. You drive the last two by hand.\n\ninstall init provider run worktree outcome integrate main one npm .vibestrate test a CLI a sentence a copy merge_ready advise your call on branch vibestrate/&lt;runId&gt;\n\n## Install and verify\n\n```\nnpm install -g vibestrate\nvibe --version\n```\n\n```\n0.2.1\n```\n\n```\nnpm warn EBADENGINE Unsupported engine {\nnpm warn EBADENGINE   package: 'vibestrate@0.2.1',\nnpm warn EBADENGINE   required: { node: '>=24' },\nnpm warn EBADENGINE   current: { node: 'v22.22.2', npm: '10.9.7' }\nnpm warn EBADENGINE }\n```\n\n```\nnode --version\nnvm install 24 && nvm use 24    # or your own version manager\nnpm install -g vibestrate\n```"
    },
    {
    ...
    },
    {
      ...
      "terms": "1 2 3 4 40 5 a accept act actually adaptiv add advis allow alon an analysi and answer applicabl apply approv arbitrat are as assuranc async auth autonomy await back befor block bold bold-lovelac branch brief budget build builder can cas cd chain challenger cheap check checkout claud claude-cod cli cod code_writ codex com command commit config configur confirm consol consult control cover crew dashboard data data-stor decis default did diff do doctor doe each end engineer error every exampl executor express fast fil finish first fix flow for from gap gat get getting-start git going gpt gpt-5 handl handler harden head her high hub id implementer in init install intak integrat interactiv into is it its just kebab kebab-cas keep list loc log lovelac m main many matcher max max-turns-run md mean merg merge-to-main model mov nam no no-consol no-select not of off on one only opin origin over own pag panel panel-review partially path pick pickup pickup-analysi pickup-review plan plan-only plan-worthy policy postgr power prefer prefer-async preset print proc profil project propos provider qualify quality quality-arbitrat quest read read-only read_only ready refresh refus requir review reviewer rewrit roadmap rol rul run s saga sav scaffold seat seat-rol second second-opin security security-review select sequenc set setup ship show simplify singl so spec spec-up specifi staff staff-engineer stag stand start statu stop stor strict structur stuck supervisor tak task tenant termin that the then thi thorough to token tracker tru turn two typ ui under under-specifi unknown unsaf unverifi up validat verbatim verifi verificat verifier vers vib vibestrat view walkthrough what when why will with workspac worktre worthy would writ yml you your your-project yourself",
      "body": "Vibestrate drives the AI coding CLIs already installed on your machine - Claude Code, Codex, Gemini, Aider, Ollama and others - as one pipeline over a single task. A **run** is one pass over that task: it opens a **worktree**, a second checkout of your repository in a separate folder, works there on a branch of its own, and hands the result back to you. Each run gets a docker-style id like `bold-lovelace`, used verbatim as the branch suffix and the worktree folder name.\n\nThis page is the tour of the whole product: the dashboard, the flows and crews behind every run, the rules you write yourself, the spec-up intake a greenfield brief triggers, and the merge path that carries a change to main. It assumes you have Vibestrate installed, one coding CLI connected, and `vibe init` run inside a git repository (the setup command: it writes the `.vibestrate/` folder this page keeps referring to). The quick start does that setup and takes one task through to a merged change, so start there if `vibe --version` prints nothing.\n\nFinish this page and you can pick a flow per task, split a run across two providers so the model judging the diff isn't the model that wrote it, write rules the reviewer enforces, and take a finished run to main yourself.\n\n## What a run actually does\n\n```\nvibe run \"Add structured logging to the settings save handler\"\n```\n\n## The dashboard\n\n```\nvibe ui\n```\n\n```\ncd /path/to/your-project && vibe ui\n```\n\n## Crews, flows and the Flow Hub\n\n```\nvibe flows list\nvibe flows show default --crew default\nvibe run \"Harden the token refresh path\" --flow security-review\n```"
    },
    {
    ...
    },
    {
      ...
      "terms": "a add advanc agent ai already and api audit automat boundary build call chosen cli cod codex consol consult context context-url control crew default doc end environment every express fin flow full get getting-start git go hav her http http-api id in index init input is it leav link log miss model modul next no no-console-log nod node_modul not of off on one only open per plan profil project quest quick quickstart read read-only recip record review rul run scaffold set shar skill start statu stay steer task the thi to together ui unattend url user validat validate-input venv verify vib vibestrat walkthrough wher work worktre you your",
      "body": "You have the models. Vibestrate takes over the logistics of putting several of them on one task: pasting the same context into each tool, keeping a spare checkout so a risky change can't reach your files, carrying one model's output into the next one's prompt, and catching where they drift apart.\n\nVibestrate is the frame they work inside. One plan, your rules, the gates you chose, one record of what happened. It drives the coding CLIs installed on your machine, and the final call stays yours - see why a human stays in the loop.\n\nMission Control is where you spend your time. `vibe ui` opens it on localhost. You start runs from the composer, watch the supervisor work, and see anything waiting on your approval.\n\n!Mission Control. A sidebar of sections runs down the left. The header reads Mission control, with an approvals tile showing Clear under the caption Nothing blocked and an Edit layout button. A New run composer and a Run summary panel sit below, and the Supervisor chat fills the right column, its mode switch set between Answers only and Answers and acts.\n\n## Advanced: CLI and automation\n\n```\nvibe init                                          # scaffold .vibestrate/\nvibe run \"Add audit logging to the settings flow\"  # plan, build, review, verify\nvibe status                                        # the runs in this project\nvibe ui                                            # open Mission Control\n```"
    },
    {
    ...
    },
    {
      ...
      "terms": "a act advanc and append append-only artifact as automat bold bold-lovelac broker chang cli cod command cost creat current dashboard decis deni detail disk durat event every execut exit fil flow follow for happen id inspect inspect-progress is it jq json liv log lovelac main md merg merge_ready messag metric n ndjson new on one only open output participant past per plan profil progress prompt provider r raw read ready relat replay resolv respons result review rol run runtim runtime-metric s sav seat select snapshot sourc stat statu stderr stdout step step-id stream termin the thi timelin to token transit txt typ ui validat validation-result verdict verificat verify vib vibestrat watch wher workflow",
