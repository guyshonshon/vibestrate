...
export const HANDBOOK_CORPUS: HandbookCorpus = {
  ...
  "entries": [
    ...
    },
    {
      ...
      "terms": "4317 a abort add advanc all and approv audit automat banner between block board brand c canva chang claud cli cmd codebas config control crew ctrl ctrl-c ctrl-k dashboard deeper did diff do doctor doe els env execut fail fil fix flow for g gat going headless hero http id in inspect inspector it jump k know ledger list liv loc localhost log main merg merge-to-main metric miss mor most network no no-open not noth open orchestrator outcom p pag policy port profil project propos provider r reach read rol run s saf setup simpl sourc start statu steer step stop supervisor switcher tab the tip to tre ui undo undo-merg use vib vibestrat watch what will word you",
    ...
    },
    {
      ...
      "titleTerms": "consult",
      "terms": "a about advanc an and answer api apply ask assist at automat beyond block can caveat chang claud cli concept confidenc config confirm consult context cost deeper did diff do documentat doe effort estimat fil fit flow get git going good ground guid heavier her i id in is it its key know last leav left level list materi md me model money new noth null one pass policy post profil project propos provider quest really reject relat run s screen shell should simpl sourc spend src task that the their ther thi tip to ts two until updat use vib vibestrat week what wher which why word writ yml you your",
    ...
    },
    {
      ...
      "titleTerms": "profil",
      "terms": "1 4 5 6 a about add agent allow and another api at balanc budget built built-in by catalog cheap claud claude-balanc claude-cheap claude-max cli codex codex-fast concept config configur cor cross cross-vendor deeper delet detect dial did disallow duplicat effort exist expensiv fast fenc from get going gpt gpt-5 guard high how id implement in is issu its itself knob know label legibility level list loc low mak max model ms must nest no not off one opt opu orchestrat order overlay patch per per-profil planner post power preset profil project provider reach real refus remov review rol run s sav say set shell simpl sit slower sonnet sourc spend statu step step-profil strict strict-writer strong sub sub-agent task that the ther thi timeout tip token tool used vendor vib vibestrat when wher word would writ writer yml you",
    ...
    },
    {
      ...
      "terms": "0 169 254 443 512 a admin agent all allow allowlist an and anthropic anthropic_api_key anthropic_base_url api aqf are auth aws back backend bas becom blast bomb by cap cap-drop cap_net_admin carry claud cli clos cod codex com concept config confin connect container credenti cross deeper default degrad deny did disposabl do docker doe drop egress enetunreach enforcement entirely environment every exampl exec execut f fail fail-clos fall filesystem filter fit for fork fork-bomb fresh gateway github github_token going guard hard harden hom host how http http_proxy https_proxy imag in insid intern is isolat it json key know label latest leak limit loc local-worktre localhost machin manag max mod model mov must my my-org nam narrow need net network never new no no-new-privileg not npmj of off on onc one only open openai openai_api_key operat opt opt-in org out outsid own per permit pid pids-limit pretend privileg process project provider provider-auth proxy prun ps radiu rather reach read read-only read-writ readonly refu registry rm root rul run s safety sandbox secret security security-opt servic set short simpl so ssh stop than the tip tmp to token too tru turn unattend unavailabl url variabl vib vibestrat vibestrate-agent vm wall want what when wher word worktre worth writ writabl yml you your",
    ...
    },
    {
      ...
      "terms": "a an and answer anywher at author beyond built button can check concept consult control crew dashboard deep deeper did do doe drop exampl exist first flow for generat going guid how i in into is it know link mak me mov navigat not noth on one only or pag panel point policy press right ring run screen set show simpl someth spot step supervisor tak that the ther through tim tip to tour turn under vibestrat walk walkthrough when wher word work written wrong you",
    ...
    },
    {
      ...
      "terms": "1 2 3 4 a about access add add-skill agent already an and anti anti-pattern api api-convent arg attach auth auth-convent be beat body bound bullet check claud command convent creat crew deeper default descript did directory discover editor exampl explicitly extend fa fil flat for form going good grant hav id in inspect is it json keep know list mak mark markdown mcp md ment mkdir nam not of one only option or p pattern payment permiss pg pg-mcp planner plu postgr prefer profil project prompt query read read-only reason requir right rol rul run sav seat sentenc server sess shap short show simpl skill specific src stat stay surfac that the thi tip titl to ts two under use vib vibestrat was way we what when which with word writ x yml you",
    ...
    },
    {
      ...
      "terms": "a add ai and api architect auth behind big big-pictur builder but chair challenger check cli cod crew deeper default did don each executor fil fill flow for get getting-start going handler has hold how http http-api implementer in init it job key know label localhost localhost-proxy log machin mean meet model mor need no non of on one open or order overrid own pick pictur profil provider proxy reviewer rol routin run s sav seat seat-rol senior senior-reviewer server set simpl start strong structur task team than that the thi through tighten tip to tool up vib vibestrat want what will with word worker you your",
    ...
    },
    {
      ...
      "terms": "a add advanc advis already and apply as ask automat befor branch clean cli click commit confirmat conflict dat deeper did do doe down env every explor ff fil finish for git git-tree-merg going good graph guid head her history id in inspect integrat into is it know last let log main merg mov need never no no-ff not of one only open or path predict propos push read read-only redact remov resolv run run-id see shap simpl sourc supervisor target the thi tip to token tre typ undo up vib what whol with word workflow would you your",
