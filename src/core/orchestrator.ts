...
import { makeEmptyMetrics } from "./metrics/runtime-metrics.js";
import {
  computeRunSpendUsd,
...
import type { ReviewSkipEvidence } from "./run/merge-readiness.js";
...
import {
  runRoleTurn,
  type RoleTurnDeps,
} from "./run-engine/role-turn.js";
import { type ResilientProviderDeps } from "./run-engine/resilient-provider.js";
import {
...
export class Orchestrator {
  ...
  private async runRole(input: {
    ...
    return runRoleTurn(this.roleTurnDeps(), input);
  }

  /** The state a role turn borrows, assembled fresh so live fields - the
   *  broker, the notifier, the execution strategy, the crew, the prompt blocks
   *  built at run start - are the ones run() wired. `turnState` is passed by
   *  reference on purpose: it holds the abort latch other call paths also
   *  write. */
  private roleTurnDeps(): RoleTurnDeps {
    return {
      projectRoot: this.projectRoot,
      config: this.config,
      rules: this.rules,
      ...
      materializedContext: this.materializedContext,
      execStrategy: this.execStrategy,
      activeCrewId: this.activeCrewId,
      codebaseMapBlock: this.codebaseMapBlock,
      ledgerPromptBlock: this.ledgerPromptBlock,
      ledgerFlagsBlock: this.ledgerFlagsBlock,
      methodologyBlock: this.methodologyBlock,
      hasStagedCodebaseMapContext: this.hasStagedCodebaseMapContext,
      broker: this.broker!,
      budgetGovernor: this.budgetGovernor,
      notify: (draft) => this.notify?.(draft),
      onProgress: (message) => this.onProgress(message),
      turnState: this.turnState,
      approvalGateDeps: () => this.approvalGateDeps(),
      resilienceDeps: () => this.resilienceDeps(),
      throwIfAbortRequested: (ctx) => this.throwIfAbortRequested(ctx),
      defaultPromptName: (index, roleId) => this.defaultPromptName(index, roleId),
    };
  }
