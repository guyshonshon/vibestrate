...
export function assertExecutableContext(ctx: AccessContext): void {
  if (ctx.profile.allowWrite && ctx.profile.cwd !== "worktree") {
    throw new PolicyError(
      `Agent "${ctx.roleId}" has write permissions but is configured to run in ${ctx.profile.cwd}. Write-enabled agents must run inside the worktree.`,
    );
  }

  if (ctx.profile.cwd === "worktree" && !ctx.worktreePath) {
    throw new PolicyError(
      `Agent "${ctx.roleId}" requires a worktree but none has been prepared.`,
    );
  }
}
...
export function resolveCwd(ctx: AccessContext): string {
  if (ctx.profile.cwd === "worktree") {
    if (!ctx.worktreePath) {
      throw new PolicyError(`Worktree not prepared for agent "${ctx.roleId}".`);
    }
