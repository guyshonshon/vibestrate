...
export function Composer({
  ...
  const usesOllama =
    provider === "ollama" ||
    (provider === "" && defaultProviderId === "ollama") ||
    Object.values(guideSlotProviders).some((p) => p === "ollama");
      ...
      <div className="px-6 py-4">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-0.5 text-[13px] text-amaco-fg">
              Define the task, choose the flow, assign CLIs, then start the run.
            </div>
          </div>
          <div className="inline-flex items-center gap-2 amaco-mono text-[10.5px] text-amaco-fg-muted">
            <Kbd>⌘K</Kbd>
            <span>focus</span>
            <span aria-hidden>·</span>
            <Kbd>↵</Kbd>
            <span>{dispatchVerb ?? "run"}</span>
            <span aria-hidden>·</span>
            <Kbd>⇧↵</Kbd>
            <span>newline</span>
          </div>
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.05fr)_minmax(24rem,0.95fr)]">
          <div className="min-w-0">
            <div className="mb-2 flex items-center gap-2 text-[12px]">
              <span className="amaco-mono text-[10.5px] uppercase tracking-[0.14em] text-amaco-accent">
                01 task
              </span>
              <span className="text-amaco-fg-muted">
                What should the orchestrator send into the selected flow?
              </span>
            </div>
            <div className="flex items-stretch gap-3">
              ...
              <div className="relative flex-1">
                <textarea
                  ...
                  placeholder="Describe the feature, bug, refactor, or review target. Slash commands still work, for example /help."
                  className="min-h-36 w-full resize-y rounded-md border border-amaco-border bg-amaco-panel px-3 py-2.5 text-[14px] leading-[1.5] text-amaco-fg placeholder:text-amaco-fg-muted focus:border-amaco-accent focus:outline-none focus:ring-1 focus:ring-amaco-accent/40 disabled:opacity-60"
                />
                <div
            ...
            {parseError ? (
              <div role="alert" className="mt-2 text-[12px] text-amaco-fail">
                {parseError}
              </div>
            ) : null}
            ...
            <button
              ...
              aria-label={selectedGuide ? "Run selected flow" : "Run"}
              className="mt-3 inline-flex items-center gap-1.5 rounded-md border border-amaco-accent/50 bg-amaco-accent/15 px-4 py-2.5 text-[13px] font-medium text-amaco-accent hover:bg-amaco-accent/25 focus:outline-none focus:ring-1 focus:ring-amaco-accent disabled:opacity-50"
            >
              <Play className="h-3.5 w-3.5" strokeWidth={2} aria-hidden />
              {busy ? "Starting..." : selectedGuide ? "Run Flow" : "Run"}
            </button>
          </div>

          <aside
            aria-label="Flow and agent controls"
            className="min-w-0 rounded-md border border-amaco-border bg-amaco-panel/45"
          >
            <div className="border-b border-amaco-border-soft px-3 py-2">
              <div className="flex items-center gap-2">
                ...
                />
                <div>
                  <div className="amaco-mono text-[10.5px] uppercase tracking-[0.14em] text-amaco-accent">
                    02 flow template
                  </div>
                  <div className="text-[11.5px] text-amaco-fg-muted">
                    Pick the orchestration recipe before the run starts.
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 px-3 py-3">
              <div>
                <div className="mb-2 flex items-center gap-2">
                  <ListChecks
                    className="h-3.5 w-3.5 text-amaco-fg-muted"
                    strokeWidth={1.6}
                    aria-hidden
                  />
              ...
              {usesOllama ? (
                <div className="rounded border border-amaco-warn/30 bg-amaco-warn/5 px-2.5 py-2 text-[11.5px] text-amaco-warn">
                  Ollama is local. Keep the Ollama app/server running and pull
                  the model used by the provider, for example{" "}
                  <span className="amaco-mono">ollama pull qwen3.5</span>.
                </div>
              ) : selectedProvider && selectedProvider.confidence !== "ready" ? (
                <div className="rounded border border-amaco-border-soft bg-amaco-panel/70 px-2.5 py-2 text-[11.5px] text-amaco-fg-dim">
                  {selectedProvider.label} is configured as a starter provider.
                  Run{" "}
                  <span className="amaco-mono text-amaco-fg">
                    amaco provider test {selectedProvider.id}
                  </span>{" "}
              ...
              {selectedGuide ? (
                ...
                <DefaultFlowPreview provider={provider || defaultProviderId || ""} />
              )}
            </div>
          </aside>
        </div>
  ...
  return (
    <label className="inline-flex items-center gap-1.5 rounded border border-amaco-border bg-amaco-panel px-2 py-1 text-amaco-fg-dim">
      ...
      <select
        ...
        aria-label="Default CLI provider"
      >
        {items.map((p) => (
...
const DEFAULT_FLOW_STEPS = [
  { label: "Plan", agent: "planner", mode: "read" },
  { label: "Architecture", agent: "architect", mode: "read" },
  { label: "Implement", agent: "executor", mode: "write" },
  { label: "Validate", agent: "validator", mode: "shell" },
  { label: "Review", agent: "reviewer", mode: "read" },
  { label: "Fix loop", agent: "fixer", mode: "write" },
  { label: "Verify", agent: "verifier", mode: "read" },
];

function DefaultFlowPreview({ provider }: { provider: string }) {
  return (
    <div className="rounded border border-amaco-border bg-amaco-panel">
      ...
      <ol className="divide-y divide-amaco-border-soft">
        {DEFAULT_FLOW_STEPS.map((step, index) => (
          <li
            key={step.label}
            className="grid gap-1 px-2.5 py-1.5 text-[11.5px] text-amaco-fg-dim sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center"
          >
            ...
            <span className="amaco-mono text-amaco-fg-muted">{step.mode}</span>
          </li>
        ))}
      </ol>
      <div className="border-t border-amaco-border-soft px-2.5 py-2 text-[11.5px] text-amaco-fg-dim">
        CLI routing:{" "}
        <span className="amaco-mono text-amaco-fg">
          {provider || "agent defaults"}
        </span>
        . Selected skills attach to every agent for this run.
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded border border-amaco-border bg-amaco-panel">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <span className="mr-2 mt-2 rounded border border-amaco-accent/35 bg-amaco-accent/10 px-2 py-1 amaco-mono text-[10.5px] text-amaco-accent">
          template controls
        </span>
      </div>

      <div className="grid gap-3 border-t border-amaco-border-soft p-2.5">
        <div className="space-y-2">
          <label className="block">
            ...
            <textarea
              ...
              className="mt-1 w-full resize-y rounded border border-amaco-border bg-amaco-panel-2 px-2.5 py-2 text-[12px] leading-[1.45] text-amaco-fg placeholder:text-amaco-fg-muted focus:border-amaco-accent focus:outline-none focus:ring-1 focus:ring-amaco-accent/40"
            />
          </label>

          <label className="inline-flex items-center gap-1.5 rounded border border-amaco-border bg-amaco-panel-2 px-2 py-1 text-[11.5px] text-amaco-fg-dim">
            <span className="amaco-mono text-[10.5px] uppercase tracking-[0.1em] text-amaco-fg-muted">
              context
            </span>
            <select
          ...
          <div className="space-y-1.5">
            {slots.map(([slotId, slot]) => (
              <label
                key={slotId}
                className="grid gap-1 rounded border border-amaco-border bg-amaco-panel-2 px-2 py-1.5 text-[11.5px] sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
              >
                <span className="min-w-0">
