...
describe("replaceSteps / replaceSeats (structural edits)", () => {
  it("replaces the whole ordered step list", () => {
    ...
    const verdict = mergeFlowPatch(base, {
      replaceSteps: [
        { id: "plan", label: "Plan", kind: "agent-turn", seat, inputs: [], outputs: [], needs: [], optional: false, skipWhenReadOnly: false, continueOnError: false, retries: 0 },
        { id: "review", label: "Review", kind: "review-turn", seat, inputs: [], outputs: [], needs: [], optional: false, skipWhenReadOnly: false, continueOnError: false, retries: 0 },
      ],
    });
    if (!verdict.ok) throw new Error(verdict.reasons.join(", "));
  ...
  it("replaces the seat map wholesale alongside the steps", () => {
    ...
    const verdict = mergeFlowPatch(base, {
      ...
      replaceSteps: [
        { id: "do", label: "Do", kind: "agent-turn", seat: "solo", inputs: [], outputs: [], needs: [], optional: false, skipWhenReadOnly: false, continueOnError: false, retries: 0 },
      ],
    });
    if (!verdict.ok) throw new Error(verdict.reasons.join(", "));
