...
async function makeClaudeFlowRepo(): Promise<{
  projectRoot: string;
  argvLog: string;
}> {
...
describe("Flow Phase 3 participant sessions", () => {
  it("opens and resumes distinct Claude Code sessions by Flow slot", async () => {
    const { projectRoot, argvLog } = await makeClaudeFlowRepo();
    const loaded = await loadConfig(projectRoot);
    const snapshot = resolveFlow({
      flow: qualityArbitrationFlow,
      source: { kind: "builtin", ref: qualityArbitrationFlow.id },
      config: loaded.config,
      task: "Exercise Flow participant sessions.",
    });
