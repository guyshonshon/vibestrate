...
import { exportGuideArbitrationDataset } from "../../src/guides/runtime/guide-arbitration-export.js";
import { guideArbitrationLedgerSchema } from "../../src/guides/runtime/guide-arbitration.js";
...
async function makeArbitrationRepo(): Promise<string> {
  const projectRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "amaco-guides-phase4-"),
  );
...
const block = (value) => [
  ...
  "AMACO_GUIDE_OUTPUT_END",
].join("\\n");
process.stdin.on("end", () => {
  if (prompt.includes("Guide step: Plan Review (plan-review)")) {
    console.log("# Plan Review\\n\\nDECISION: APPROVED\\n\\n" + block({
      contract: "amaco.guide.findings.v1",
      stepId: "plan-review",
      findings: []
    }));
  } else if (prompt.includes("Guide step: Implementation Review (implementation-review)")) {
    console.log("# Review\\n\\nDECISION: APPROVED\\n\\n" + block({
      contract: "amaco.guide.findings.v1",
      ...
      findings: [{
        ...
        evidence: [{ kind: "diff", ref: "artifacts/guides/implement/diff-snapshot.json" }],
        recommendation: "Add or verify a regression test."
      }]
    }));
  } else if (prompt.includes("Guide step: Second Review (second-review)")) {
    console.log("# Second Review\\n\\nDECISION: APPROVED\\n\\n" + block({
      contract: "amaco.guide.finding-resolutions.v1",
      stepId: "second-review",
      resolutions: [{
        findingId: "finding-tests",
        disposition: "resolved",
        rationale: "The builder attached validation evidence.",
        evidence: [{ kind: "validation", ref: "artifacts/guides/validation/validation-results.json" }]
      }]
    }));
  } else if (prompt.includes("Guide step: Challenge Response (challenge-response)")) {
  ...
  } else if (prompt.includes("Guide step: Decision Summary (decision-summary)")) {
    ...
    console.log("# Unhandled Guide turn");
  }
});
`,
    { mode: 0o755 },
...
describe("Guide Phase 4 arbitration records", () => {
  it("parses evidence records, creates accepted-finding review work, and exports data", async () => {
    ...
    const ledger = guideArbitrationLedgerSchema.parse(
    ...
    expect(suggestions[0]).toMatchObject({
      source: "artifact",
      sourceArtifactPath:
        "artifacts/guides/implementation-review/output.md",
    });
    ...
    ).resolves.toContain("AMACO_GUIDE_OUTPUT:");
    await expect(
      fs.readFile(path.join(runDir, "events.ndjson"), "utf8"),
    ).resolves.toContain('"guide.decision.completed"');

    const exported = await exportGuideArbitrationDataset({
      projectRoot,
      runId: result.runId,
    });
